...
public class GrAliasAnnotationChecker extends CustomAnnotationChecker
{
	...
	public boolean checkArgumentList(@NotNull AnnotationHolder holder, @NotNull GrAnnotation annotation)
	{
		...
		final Set<String> usedAttributes = GrAnnotationCollector.collectAnnotations(annotations, annotation,
				annotationCollector);
		...
		if(attributes.length == 1 && attributes[0].getNameIdentifierGroovy() == null && !usedAttributes.contains
				("value"))
		{
			holder.createErrorAnnotation(attributes[0], GroovyBundle.message("at.interface.0.does.not.contain" +
					".attribute", aliasQName, "value"));
		}
		...
		for(GrAnnotationNameValuePair pair : attributes)
		{
			...
			{
				holder.createErrorAnnotation(nameIdentifier, GroovyBundle.message("at.interface.0.does.not.contain" +
						".attribute", aliasQName, pair.getName()));
			}
		}
		return true;
	}
