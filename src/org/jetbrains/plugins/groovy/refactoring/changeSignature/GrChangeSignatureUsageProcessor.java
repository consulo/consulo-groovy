...
{
	private static final Logger LOG = Logger.getInstance("#org.jetbrains.plugins.groovy.refactoring.changeSignature" +
			".GrChangeSignatureUsageProcessor");
		...
		for(UsageInfo usageInfo : refUsages.get())
		{
			if(usageInfo instanceof GrMethodCallUsageInfo)
			{
				...
				if(methodCallUsageInfo.isToChangeArguments())
				{
					...
					if(needDefaultValue && (caller == null || referencedMethod == null || !MethodSignatureUtil.isSuperMethod(referencedMethod,
							caller)))
					{
	...
	private static boolean processPrimaryMethodInner(JavaChangeInfo changeInfo, GrMethod method, @Nullable PsiMethod baseMethod)
	{
		...
		{
			String newName = baseMethod == null ? changeInfo.getNewName() : RefactoringUtil.suggestNewOverriderName(method.getName(),
					baseMethod.getName(), changeInfo.getNewName());
		...
		for(JavaParameterInfo newParameter : newParameters)
		{
			...
			GrParameter grParameter = factory.createParameter(newName, type == null ? null : type.getCanonicalText(), getInitializer(newParameter),
					parameterList);
		...
		else
		{
			if(usageInfo instanceof GrMethodCallUsageInfo)
			{
				processMethodUsage(element, ((JavaChangeInfo) changeInfo), ((GrMethodCallUsageInfo) usageInfo).isToChangeArguments(),
						((GrMethodCallUsageInfo) usageInfo).isToCatchExceptions(), ((GrMethodCallUsageInfo) usageInfo).getMapToArguments(),
	...
	private static void processClassUsage(GrTypeDefinition psiClass, JavaChangeInfo changeInfo)
	{
		...
		GrMethod constructor = GroovyPsiElementFactory.getInstance(psiClass.getProject()).createConstructorFromText(name,
				ArrayUtil.EMPTY_STRING_ARRAY, ArrayUtil.EMPTY_STRING_ARRAY, "{}", null);
	...
	private static void processConstructor(GrMethod constructor, JavaChangeInfo changeInfo)
	{
		...
		processMethodUsage(invocation.getInvokedExpression(), changeInfo, changeInfo.isParameterSetOrOrderChanged() || changeInfo
				.isParameterNamesChanged(), changeInfo.isExceptionSetChanged(), GrClosureSignatureUtil.ArgInfo.<PsiElement>empty_array(),
				substitutor);
	}
	...
	private static void processMethodUsage(PsiElement element, JavaChangeInfo changeInfo, boolean toChangeArguments, boolean toCatchExceptions,
			GrClosureSignatureUtil.ArgInfo<PsiElement>[] map, PsiSubstitutor substitutor)
	{
		if(map == null)
		{
			return;
			...
			for(int i = 0; i < parameters.length; i++)
			{
				...
				if(index >= 0)
				{
					...
					{ //arguments for Map and varArg
						if((i != 0 || !(arguments.size() > 0 && arguments.iterator().next() instanceof GrNamedArgument)) && (i != parameters.length
								- 1 || !parameter.isVarargType()))
						{
							final PsiType type = parameter.createType(changeInfo.getMethod().getParameterList(), argumentList.getManager());
							final GrExpression arg = GroovyRefactoringUtil.generateArgFromMultiArg(substitutor, arguments, type,
									element.getProject());
					...
					try
					{
						...
						if(i > 0 && (value == null || anchor == null))
						{
							PsiElement comma = Factory.createSingleLeafElement(GroovyTokenTypes.mCOMMA, ",", 0, 1,
									SharedImplUtil.findCharTableByTree(argumentList.getNode()), argumentList.getManager()).getPsi();
							if(anchor == null)
							{
	...
	private static void fixExceptions(PsiElement element, PsiClassType[] exceptions)
	{
		...
		final GroovyPsiElement context = PsiTreeUtil.getParentOfType(element, GrTryCatchStatement.class, GrClosableBlock.class, GrMethod.class,
				GroovyFile.class);
		...
		else if(context instanceof GrTryCatchStatement)
		{
			...
			final List<PsiClassType> psiClassTypes = filterOutExceptions(exceptions, context,
	...
	private static PsiElement generateTryCatch(PsiElement element, PsiClassType[] exceptions)
	{
		...
		GrTryCatchStatement tryCatch = (GrTryCatchStatement) GroovyPsiElementFactory.getInstance(element.getProject()).createStatementFromText
	...
	private static List<PsiClassType> filterOutExceptions(PsiClassType[] exceptions, final GroovyPsiElement context,
