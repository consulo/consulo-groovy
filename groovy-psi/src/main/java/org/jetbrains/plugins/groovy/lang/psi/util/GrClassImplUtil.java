...
public class GrClassImplUtil {
  ...
  public static PsiClass getBaseClass(GrTypeDefinition grType) {
    if (grType.isEnum()) {
      return JavaPsiFacade.getInstance(grType.getProject()).findClass(CommonClassNames.JAVA_LANG_ENUM,
                                                                      grType.getResolveScope());
    }
    else {
      return JavaPsiFacade.getInstance(grType.getProject()).findClass(CommonClassNames.JAVA_LANG_OBJECT,
                                                                      grType.getResolveScope());
    }
  }
  ...
  public static PsiClassType[] getExtendsListTypes(GrTypeDefinition grType) {
    PsiClassType[] extendsTypes = getReferenceListTypes(grType.getExtendsClause());
    ...
    for (PsiClassType type : extendsTypes) {
      ...
      if (superClass instanceof GrTypeDefinition && !superClass.isInterface() || superClass != null &&
        GroovyCommonClassNames.GROOVY_OBJECT_SUPPORT.equals(superClass.getQualifiedName())) {
        return extendsTypes;
      }
    }

    PsiClass grObSupport = GroovyPsiManager.getInstance(grType.getProject()).findClassWithCache
      (GroovyCommonClassNames.GROOVY_OBJECT_SUPPORT, grType.getResolveScope());
    if (grObSupport != null) {
      PsiClassType type = JavaPsiFacade.getInstance(grType.getProject()).getElementFactory().createType
        (grObSupport);
  ...
  private static void getImplementListsInner(GrTypeDefinition grType,
                                             List<PsiClassType> result,
                                             Set<PsiClass> visited) {
    if (!visited.add(grType)) {
      return;
    }

    PsiClassType[] implementsTypes = getReferenceListTypes(grType.getImplementsClause());
  ...
  @Nullable
  private static List<PsiClassType> getImplementsFromDelegate(@Nonnull final GrTypeDefinition grType,
    ...
    return RecursionManager.doPreventingRecursion(grType, true, new Computable<List<PsiClassType>>() {
      ...
      public List<PsiClassType> compute() {
        List<PsiClassType> result = new ArrayList<PsiClassType>();
        GrField[] fields = grType.getCodeFields();
        for (GrField field : fields) {
          ...
          boolean shouldImplement = shouldImplementDelegatedInterfaces(delegate);
          if (!shouldImplement) {
            continue;
          }
  ...
  public static PsiClassType getGroovyObjectType(@Nonnull PsiElement context) {
  ...
  public static PsiMethod[] getAllMethods(final GrTypeDefinition grType) {
    return LanguageCachedValueUtil.getCachedValue(grType, new CachedValueProvider<PsiMethod[]>() {
      @Nullable
      @Override
  ...
  public static boolean processDeclarations(@Nonnull GrTypeDefinition grType,
                                            @Nonnull PsiScopeProcessor processor,
                                            @Nonnull ResolveState state,
                                            @Nullable PsiElement lastParent,
                                            @Nonnull PsiElement place) {
    if (place instanceof GrCodeReferenceElement && lastParent instanceof GrModifierList) {
      PsiElement possibleAnnotation = PsiTreeUtil.skipParentsOfType(place, GrCodeReferenceElement.class);
    ...
    boolean processInstanceMethods = (ResolveUtil.shouldProcessMethods(classHint) || ResolveUtil
      .shouldProcessProperties(classHint)) && shouldProcessInstanceMembers(grType, lastParent);
    ...
    if (ResolveUtil.shouldProcessProperties(classHint)) {
      ...
      if (name != null) {
        ...
        if (fieldInfo != null) {
          if (!processField(grType, processor, state, place, processInstanceMethods, substitutor, factory,
                            level, fieldInfo)) {
            return false;
          }
        }
          ...
          if (field != null && field.hasModifierProperty(PsiModifier.PUBLIC)) {
            if (!processField(grType, processor, state, place, processInstanceMethods, substitutor,
                              factory, level, new CandidateInfo(field, PsiSubstitutor.EMPTY))) {
              return false;
            }
          }
        }
      ...
      if (name == null) {
        for (List<CandidateInfo> list : methodsMap.values()) {
          for (CandidateInfo info : list) {
            if (!processMethod(grType, processor, state, place, processInstanceMethods, substitutor,
                               factory, level, isPlaceGroovy, info)) {
              return false;
            }
          }
        }
    ...
    GrTypeDefinitionBody body = grType.getBody();
    if (body != null) {
  ...
  private static boolean processField(@Nonnull GrTypeDefinition grType,
                                      @Nonnull PsiScopeProcessor processor,
                                      @Nonnull ResolveState state,
                                      @Nonnull PsiElement place,
    ...
    PsiSubstitutor finalSubstitutor = PsiClassImplUtil.obtainFinalSubstitutor(field.getContainingClass(),
  ...
  private static boolean processMethod(@Nonnull GrTypeDefinition grType,
                                       @Nonnull PsiScopeProcessor processor,
                                       @Nonnull ResolveState state,
                                       @Nonnull PsiElement place,
                                       boolean processInstanceMethods,
    ...
    if (!processInstanceMember(processInstanceMethods, method) || isSameDeclaration(place,
                                                                                    method) || !isMethodVisible(placeGroovy, method)) {
      return true;
    }
    LOG.assertTrue(method.getContainingClass() != null);
    PsiSubstitutor finalSubstitutor = PsiClassImplUtil.obtainFinalSubstitutor(method.getContainingClass(),
                                                                                    info.getSubstitutor(),
                                                                                    grType,
                                                                                    substitutor,
                                                                                    factory,
  ...
  private static boolean shouldProcessInstanceMembers(@Nonnull GrTypeDefinition grType,
                                                      @Nullable PsiElement lastParent) {
    if (lastParent != null) {
      GrModifierList modifierList = grType.getModifierList();
      if (modifierList != null && modifierList.findAnnotation(GroovyCommonClassNames.GROOVY_LANG_CATEGORY) !=
        null) {
        return false;
      }
    }
  ...
  private static boolean processInstanceMember(boolean shouldProcessInstance, @Nonnull PsiMember member) {
  ...
  @Nonnull
  private static List<PsiClass> getInnerClassesForResolve(@Nonnull final GrTypeDefinition grType,
    ...
    List<PsiClass> classes = RecursionManager.doPreventingRecursion(grType, true, new Computable<List<PsiClass>>() {
      ...
      public List<PsiClass> compute() {
        ...
        for (CandidateInfo info : CollectClassMembersUtil.getAllInnerClasses(grType, false).values()) {
          ...
          if (lastParent == null || !containingClass.isInterface() || PsiTreeUtil.isAncestor(containingClass,
                                                                                             place, false)) {
  ...
  public static boolean isSameDeclaration(PsiElement place, PsiElement element) {
    ...
    while (place != null) {
      ...
      if (place instanceof GrEnumConstantInitializer) {
        return false;
      }
    }
    return false;
  ...
  private static PsiMethod[] findMethodsByName(GrTypeDefinition grType,
    ...
    Map<String, List<CandidateInfo>> methodsMap = CollectClassMembersUtil.getAllMethods(grType,
                                                                                        includeSyntheticAccessors);
    return PsiImplUtil.mapToMethods(methodsMap.get(name));
  }

  @Nonnull
  public static PsiMethod[] findMethodsBySignature(GrTypeDefinition grType,
  ...
  @Nonnull
  public static PsiMethod[] findCodeMethodsBySignature(GrTypeDefinition grType,
                                                       PsiMethod patternMethod,
                                                       boolean checkBases) {
  ...
  @Nullable
  private static MethodSignature getSignatureForInheritor(@Nonnull PsiMethod methodFromSuperClass,
                                                          @Nonnull GrTypeDefinition inheritor) {
    PsiClass clazz = methodFromSuperClass.getContainingClass();
    if (clazz == null) {
      return null;
    }
    PsiSubstitutor superSubstitutor = TypeConversionUtil.getClassSubstitutor(clazz, inheritor,
  ...
  public static List<Pair<PsiMethod, PsiSubstitutor>> findMethodsAndTheirSubstitutorsByName(GrTypeDefinition grType,
    ...
    if (!checkBases) {
      PsiMethod[] methods = grType.findMethodsByName(name, false);
  ...
  @Nullable
  public static PsiField findFieldByName(GrTypeDefinition grType,
                                         String name,
                                         boolean checkBases,
                                         boolean includeSynthetic) {
    if (!checkBases) {
  ...
  public static void addExpandingReflectedMethods(Collection<PsiMethod> result, PsiMethod method) {
    if (method instanceof GrMethod) {
      GrReflectedMethod[] reflectedMethods = ((GrMethod)method).getReflectedMethods();
  ...
  public static void collectMethodsFromBody(@Nonnull GrTypeDefinition definition, Collection<PsiMethod> result) {
