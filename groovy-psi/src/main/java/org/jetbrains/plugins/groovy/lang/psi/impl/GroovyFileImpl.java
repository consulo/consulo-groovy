...
public class GroovyFileImpl extends GroovyFileBaseImpl implements GroovyFile {
  ...
  private static final CachedValueProvider<ConcurrentMap<String, GrBindingVariable>> BINDING_PROVIDER = new
    CachedValueProvider<ConcurrentMap<String, GrBindingVariable>>() {
      ...
      public Result<ConcurrentMap<String, GrBindingVariable>> compute() {
        ConcurrentMap<String, GrBindingVariable> map = ContainerUtil.newConcurrentMap();
  ...
  public String getPackageName() {
    ...
    if (packageDef != null) {
      String name = packageDef.getPackageName();
      if (name != null) {
        return name;
      }
    }
  ...
  private GrParameter getSyntheticArgsParameter() {
    ...
    if (parameter == null) {
      PsiType psiType = JavaPsiFacade.getElementFactory(getProject()).createTypeFromText("java.lang" +
                                                                                                 ".String[]", this);
  ...
  public boolean processDeclarations(@Nonnull PsiScopeProcessor processor,
    ...
    if (!processImports(processor, state, lastParent, place, importStatements, false)) {
      return false;
    }
    if (!processDeclarationsInPackage(processor, state, lastParent, place)) {
      return false;
    }
    if (!processImports(processor, state, lastParent, place, importStatements, true)) {
      return false;
    }
    if (!GroovyImportHelper.processImplicitImports(processor, state, lastParent, place, this)) {
      return false;
    }
    ...
    if (ResolveUtil.shouldProcessProperties(classHint)) {
      ...
      if (isInScriptBody(lastParent, place)) {
        if (!processBindings(processor, state, place)) {
          return false;
        }
      }
    }
  ...
  private boolean processBindings(@Nonnull PsiScopeProcessor processor,
                                  @Nonnull ResolveState state,
                                  PsiElement place) {
    if (!isPhysical()) {
      return true;
    ...
    ConcurrentMap<String, GrBindingVariable> bindings = getBindings();

    GrBindingVariable variable = bindings.get(name);
    if (variable == null) {
      variable = Maps.cacheOrGet(bindings, name, new GrBindingVariable(this, name, null));
    }
  ...
  private ConcurrentMap<String, GrBindingVariable> getBindings() {
    return LanguageCachedValueUtil.getCachedValue(this, BINDING_PROVIDER);
  }
  ...
  private boolean processChildrenScopes(@Nonnull PsiScopeProcessor processor,
    ...
    StubElement<?> stub = getStub();
    if (stub != null) {
      return true; // only local usages are traversed here. Having a stub means the clients are outside and
      // won't see our variables
    }

    PsiElement run = lastParent == null ? getLastChild() : lastParent.getPrevSibling();
    while (run != null) {
      if (shouldProcess(lastParent, run) && !run.processDeclarations(processor, state, null, place)) {
        return false;
      }
  ...
  public GrImportStatement[] getImportStatements() {
    ...
    return calcTreeElement().getChildrenAsPsiElements(GroovyElementTypes.IMPORT_STATEMENT,
                                                      GrImportStatement.ARRAY_FACTORY);
  }
  ...
  private boolean checkIsScript() {
    GrTopStatement[] topStatements = findChildrenByClass(GrTopStatement.class);
  ...
  public void setPackageName(String packageName) {
    ASTNode fileNode = getNode();
    GrPackageDefinition currentPackage = getPackageDefinition();
    ...
    GroovyPsiElementFactory factory = GroovyPsiElementFactory.getInstance(getProject());
    GrPackageDefinition newPackage = (GrPackageDefinition)factory.createTopElementFromText("package " +
                                                                                                   packageName);
  ...
  public GrPackageDefinition setPackage(@Nullable GrPackageDefinition newPackage) {
    GrPackageDefinition oldPackage = getPackageDefinition();
    if (oldPackage == null) {
      if (newPackage != null) {
        GrPackageDefinition result = (GrPackageDefinition)addAfter(newPackage, null);
  ...
  public PsiType getInferredScriptReturnType() {
    return LanguageCachedValueUtil.getCachedValue(this, () -> CachedValueProvider.Result.create(GroovyPsiManager.inferType(
      GroovyFileImpl.this,
      new MethodTypeInferencer(GroovyFileImpl.this)), PsiModificationTracker.MODIFICATION_COUNT));
  }
  ...
  public PsiElement getOriginalElement() {
    PsiClass scriptClass = getScriptClass();
    if (scriptClass != null) {
      PsiElement originalElement = scriptClass.getOriginalElement();
