  ...
  private static class MyTypesCalculator implements Function<GrListOrMapImpl, PsiType> {
    ...
    private static PsiClassType inferMapInitializerType(GrListOrMapImpl listOrMap) {
      ...
      if (namedArgs.length == 0) {
        ...
        if (lType instanceof PsiClassType && InheritanceUtil.isInheritor(lType, CommonClassNames.JAVA_UTIL_MAP)) {
          ...
          if (hashMap == null) {
            hashMap = facade.findClass(CommonClassNames.JAVA_UTIL_MAP, scope);
          }
          if (hashMap != null) {
            PsiSubstitutor mapSubstitutor = PsiSubstitutor.EMPTY.
                                                                  put(hashMap.getTypeParameters()[0],
                                                                      com.intellij.java.language.psi.util.PsiUtil
                                                                        .substituteTypeParameter(lType,
                                                                                                 CommonClassNames.JAVA_UTIL_MAP,
                                                                                                 0,
                                                                                                 false)).
                                                                  put(hashMap.getTypeParameters()[1],
                                                                      com.intellij.java.language.psi.util.PsiUtil
                                                                        .substituteTypeParameter(lType,
                                                                                                 CommonClassNames.JAVA_UTIL_MAP,
                                                                                                 1,
    ...
    private static PsiClassType getTupleType(final GrExpression[] initializers, GrListOrMap listOrMap) {
      ...
      if (initializers.length == 0) {
        ...
        if (lType instanceof PsiClassType && InheritanceUtil.isInheritor(lType,
                                                                         CommonClassNames.JAVA_UTIL_LIST)) {
          PsiClass arrayList = facade.findClass(CommonClassNames.JAVA_UTIL_ARRAY_LIST, scope);
          if (arrayList == null) {
            arrayList = facade.findClass(CommonClassNames.JAVA_UTIL_LIST, scope);
          }
          if (arrayList != null) {
            PsiSubstitutor arrayListSubstitutor = PsiSubstitutor.EMPTY.put(arrayList.getTypeParameters()[0],
                                                                           com.intellij.java.language.psi.util.PsiUtil
                                                                             .substituteTypeParameter(lType,
                                                                                                      CommonClassNames.JAVA_UTIL_LIST,
                                                                                                      0,
