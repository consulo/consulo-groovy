...
public class GroovyParser implements PsiParser
{
	...
	public boolean parseIfStatement(PsiBuilder builder)
	{
		...
		if(GroovyTokenTypes.kELSE.equals(builder.getTokenType()) || (Separators.parse(builder) && builder.getTokenType
	...
	private void parseCodeBlock(PsiBuilder builder, TokenSet until)
	{
		while(true)
		{
			...
			if(!parseGenericStatement(builder, until))
			{
				break;
			}
		}
	...
	private boolean parseGenericStatement(PsiBuilder builder, TokenSet until)
	{
		...
		else
		{
			...
			assert builder.getTokenType() != GroovyTokenTypes.mLCURLY && builder.getTokenType() != GroovyTokenTypes
					.mRCURLY;
	...
	private boolean skipSeparators(PsiBuilder builder)
	{
		...
		while(builder.getTokenType() == GroovyTokenTypes.mSEMI || isExtendedSeparator(builder.getTokenType()) ||
				builder.getTokenType() == GroovyTokenTypes.mNLS)
		{
			hasSeparators = true;
			builder.advanceLexer();
		}
	...
	public boolean parseStatement(PsiBuilder builder, boolean isBlockStatementNeeded)
	{
		...
		if(parseDeclaration(builder, false, false, null))
		{
			return true;
		}
	...
	/**
	 * parses imports (marks them as not allowed), type definitions, methods, variables or fields (if isInClass),
	 * initializers (if isInClass), constructors
	...
	private boolean parseLabeledStatement(PsiBuilder builder)
	{
		...
		if(!ParserUtils.getToken(builder, GroovyTokenTypes.mIDENT) || !ParserUtils.getToken(builder,
