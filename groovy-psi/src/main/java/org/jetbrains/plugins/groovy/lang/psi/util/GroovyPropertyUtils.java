...
public class GroovyPropertyUtils
{
											   ...
											   boolean checkSuperClasses)
	{
		...
		for(PsiMethod method : methods)
		{
			if(isStatic != null && method.hasModifierProperty(PsiModifier.STATIC) != isStatic)
			{
				continue;
			}
	...
	{
		if(isSimplePropertyGetter(accessor))
		{
			return getPropertyNameByGetter(accessor);
		}
		if(isSimplePropertySetter(accessor))
		{
			return getPropertyNameBySetter(accessor);
		}
		return null;
	}
	...
	public static boolean isGetterName(@Nonnull String name)
	{
		...
		if(isUpperCase(name.charAt(prefixLength)))
		{
			return true;
		}
