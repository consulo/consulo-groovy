...
public class SubstitutorComputer
{
	...
	protected PsiType inferContextType()
	{
		...
		else if(parent instanceof GrAssignmentExpression && myPlaceToInferContext.equals(((GrAssignmentExpression)
				parent).getRValue()))
		{
			...
			@NotNull PsiType[] argTypes)
	{
		...
		PsiSubstitutor substitutor = myHelper.inferTypeArguments(typeParameters, parameterTypes, argumentTypes,
	...
	private PsiType handleConversion(@Nullable PsiType paramType, @Nullable PsiType argType)
	{
		...
		if(!TypesUtil.isAssignable(TypeConversionUtil.erasure(paramType), argType,
				myPlace) && TypesUtil.isAssignableByMethodCallConversion(paramType, argType, myPlace))
		{
			return paramType;
		}
		return argType;
	...
	private PsiType handleConversionOfSAMType(@Nullable PsiType samType, @NotNull PsiClassType closure)
	{
		if(samType instanceof PsiClassType)
		{
			...
			{
				MethodSignature samSignature = ClosureToSamConverter.findSingleAbstractMethod(samClass,
					...
					if(samMethod != null)
					{
						...
						if(closureArgs.length == 1 && samMethod.getReturnType() != null)
						{
							PsiSubstitutor substitutor = myHelper.inferTypeArguments(samClass.getTypeParameters(),
									new PsiType[]{samMethod.getReturnType()}, closureArgs, LanguageLevel.JDK_1_7);

							if(!substitutor.getSubstitutionMap().isEmpty())
							{
								return JavaPsiFacade.getElementFactory(myPlace.getProject()).createType(samClass,
										substitutor);
							}
						}
					}
