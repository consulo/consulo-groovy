...
public class PsiImplUtil {
  ...
  public static GrExpression replaceExpression(GrExpression oldExpr,
    ...
    if (getRegexAtTheBeginning(newExpr) != null && isAfterIdentifier(oldExpr)) {
      ...
      GrLiteral regex = getRegexAtTheBeginning(copy);
    ...
    if (oldParent instanceof GrStringInjection) {
      if (newExpr instanceof GrString || newExpr instanceof GrLiteral && ((GrLiteral)newExpr).getValue()
        instanceof String) {
        return GrStringUtil.replaceStringInjectionByLiteral((GrStringInjection)oldParent,
                                                            (GrLiteral)newExpr);
      }
      else {
    ...
    if (PsiTreeUtil.getParentOfType(oldExpr, GrStringInjection.class, false, GrCodeBlock.class) != null) {
      GrStringInjection stringInjection = PsiTreeUtil.getParentOfType(oldExpr, GrStringInjection.class);
      GrStringUtil.wrapInjection(stringInjection);
      assert stringInjection != null;
      PsiElement replaced = oldExpr.replaceWithExpression(newExpr, removeUnnecessaryParentheses);
    ...
    //if newExpr is the first grand child of command argument list we should replace command arg list with
    // parenthesised arg list.
    // In other case the code will be broken. So we try to find wrapping command arg list counting levels. After
    // arg list replace we go inside it
    ...
    if (newExpr instanceof GrParenthesizedExpression && isFirstChild(newExpr)) {
      ...
      if (element instanceof GrCommandArgumentList) {
        ...
        PsiElement parent = commandArgList.getParent();
        LOG.assertTrue(parent instanceof GrApplicationStatement);

        GrMethodCall methodCall = factory.createMethodCallByAppCall((GrApplicationStatement)parent);
        GrMethodCall newCall = (GrMethodCall)parent.replace(methodCall);
   ...
   */
  @Nullable
  private static GrExpression addParenthesesIfNeeded(GrExpression newExpr,
                                                     GrExpression oldExpr,
                                                     GrExpression oldParent) {
    GroovyPsiElementFactory factory = GroovyPsiElementFactory.getInstance(oldExpr.getProject());
  ...
  private static boolean isNotAssociative(GrBinaryExpression binaryExpression) {
    IElementType opToken = binaryExpression.getOperationTokenType();
    return !TokenSets.ASSOCIATIVE_BINARY_OP_SET.contains(opToken);
  }

  @Nullable
  @RequiredReadAction
  public static GrExpression getRuntimeQualifier(@Nonnull GrReferenceExpression refExpr) {
    GrExpression qualifier = refExpr.getQualifierExpression();
    if (qualifier != null) {
      return qualifier;
    }

    for (GrClosableBlock closure = PsiTreeUtil.getParentOfType(refExpr, GrClosableBlock.class); closure != null;
         closure = PsiTreeUtil.getParentOfType(closure, GrClosableBlock.class)) {

      PsiElement parent = closure.getParent();
      if (parent instanceof GrArgumentList argumentList) {
        parent = argumentList.getParent();
      }
      ...
      GrExpression funExpr = methodCall.getInvokedExpression();
      if (!(funExpr instanceof GrReferenceExpression funRefExpr)) {
        return funExpr;
      }

      if (!(funRefExpr.resolve() instanceof PsiMethod method)) {
        return funRefExpr;
      }

      if (method instanceof GrGdkMethod gdkMethod
        && isFromDGM(gdkMethod)
        && !GdkMethodUtil.isWithName(gdkMethod.getStaticMethod().getName())) {
        continue;
      }

      qualifier = funRefExpr.getQualifierExpression();
      if (qualifier != null) {
        return qualifier;
      }
    }
  ...
  private static boolean isFromDGM(GrGdkMethod resolved) {
    PsiClass containingClass = resolved.getStaticMethod().getContainingClass();
  ...
  public static PsiElement realPrevious(PsiElement previousLeaf) {
    while (previousLeaf != null && (previousLeaf instanceof PsiWhiteSpace ||
  ...
  private static int getExprPriorityLevel(GrExpression expr) {
    ...
    if (expr instanceof GrUnaryExpression unaryExpr) {
      priority = unaryExpr.isPostfix() ? 5 : 6;
    }
    else if (expr instanceof GrTypeCastExpression) {
      priority = 6;
    }


    else if (expr instanceof GrBinaryExpression binaryExpr) {
      IElementType opToken = binaryExpr.getOperationTokenType();

      if (opToken == GroovyTokenTypes.mSTAR_STAR) {
        priority = 7;
      }
      else if (opToken == GroovyTokenTypes.mSTAR || opToken == GroovyTokenTypes.mDIV) {
        priority = 8;
      }
      else if (opToken == GroovyTokenTypes.mPLUS || opToken == GroovyTokenTypes.mMINUS) {
        priority = 9;
      }
      else if (TokenSets.SHIFT_SIGNS.contains(opToken)) {
        priority = 10;
      }
      else if (opToken == GroovyTokenTypes.mRANGE_EXCLUSIVE || opToken == GroovyTokenTypes.mRANGE_INCLUSIVE) {
        priority = 11;
      }
      else if (TokenSets.RELATIONS.contains(opToken)) {
        priority = 12;
      }
      else if (opToken == GroovyTokenTypes.mEQUAL || opToken == GroovyTokenTypes.mNOT_EQUAL || opToken == GroovyTokenTypes.mCOMPARE_TO) {
        priority = 13;
      }
      else if (opToken == GroovyTokenTypes.mREGEX_FIND || opToken == GroovyTokenTypes.mREGEX_MATCH) {
        priority = 14;
      }
      else if (opToken == GroovyTokenTypes.mBAND) {
        priority = 15;
      }
      else if (opToken == GroovyTokenTypes.mBXOR) {
        priority = 16;
      }
      else if (opToken == GroovyTokenTypes.mBOR) {
        priority = 17;
      }
      else if (opToken == GroovyTokenTypes.mLAND) {
        priority = 18;
      }
      else if (opToken == GroovyTokenTypes.mLOR) {
        priority = 19;
      }
      else {
    ...
    else if (expr instanceof GrConditionalExpression) {
      priority = 20;
    }
    else if (expr instanceof GrSafeCastExpression) {
      priority = 21;
    }
    else if (expr instanceof GrAssignmentExpression) {
      priority = 22;
    }
    else if (expr instanceof GrApplicationStatement) {
      priority = 23;
    }
    else {
      priority = 0;
  ...
  @Nullable
  public static PsiMethod extractUniqueElement(@Nonnull GroovyResolveResult[] results) {
    if (results.length != 1) {
      return null;
    }
    PsiElement element = results[0].getElement();
    return element instanceof PsiMethod method ? method : null;
  }

  @Nonnull
  public static GroovyResolveResult extractUniqueResult(@Nonnull GroovyResolveResult[] results) {
    if (results.length != 1) {
  ...
  @Nonnull
  @RequiredReadAction
  public static String getName(@Nonnull GrNamedElement namedElement) {
    ...
    if (node.getElementType() == GroovyTokenTypes.mSTRING_LITERAL || node.getElementType() == GroovyTokenTypes
      .mGSTRING_LITERAL) {
    ...
    throw new IncorrectOperationException("incorrect name element: " + node.getElementType() + ", " +
                                            "named element: " + namedElement);
  }

  @RequiredWriteAction
  public static void removeNewLineAfter(@Nonnull GrStatement statement) {
  ...
  @RequiredWriteAction
  public static void deleteStatementTail(PsiElement container, @Nonnull PsiElement statement) {
    ...
    while (next != null) {
      ...
      if (type == GroovyTokenTypes.mSEMI || type == TokenType.WHITE_SPACE && !next.getText().contains("\n")) {
        ...
        container.getNode().replaceChild(node, Factory.createSingleLeafElement(type, substring, 0,
																																							 substring.length(), null, container.getManager()));
        return;
      }
      else {
        break;
  ...
  @RequiredWriteAction
  public static <T extends PsiElement> void setQualifier(@Nonnull GrQualifiedReference<T> ref, @Nullable T newQualifier) {
  ...
  public static boolean isSimpleArrayAccess(PsiType exprType,
                                            PsiType[] argTypes,
                                            PsiElement context,
                                            boolean isLValue) {
  ...
  public static PsiCodeBlock getOrCreatePsiCodeBlock(GrOpenBlock block) {
    ...
    SoftReference<PsiCodeBlock> ref = block.getUserData(PSI_CODE_BLOCK);
    PsiCodeBlock body = SoftReference.dereference(ref);
    if (body != null) {
      return body;
    }
  ...
  public static PsiTypeElement getOrCreateTypeElement(GrTypeElement typeElement) {
    ...
    SoftReference<PsiTypeElement> ref = typeElement.getUserData(PSI_TYPE_ELEMENT);
    PsiTypeElement element = SoftReference.dereference(ref);
    if (element != null) {
      return element;
    }
  ...
  public static PsiExpression getOrCreatePisExpression(GrExpression expr) {
    ...
    SoftReference<PsiExpression> ref = expr.getUserData(PSI_EXPRESSION);
    PsiExpression element = SoftReference.dereference(ref);
    if (element != null) {
      return element;
    }
  ...
  public static PsiReferenceList getOrCreatePsiReferenceList(GrReferenceList list, PsiReferenceList.Role role) {
    ...
    SoftReference<PsiReferenceList> ref = list.getUserData(PSI_REFERENCE_LIST);
    PsiReferenceList element = SoftReference.dereference(ref);
    if (element != null) {
      return element;
    }
    GrSyntheticReferenceList newList = new GrSyntheticReferenceList(list, role);
  ...
  @Nullable
  public static PsiAnnotation getAnnotation(@Nonnull PsiModifierListOwner field, @Nonnull String annotationName) {
  ...
  public static PsiType inferExpectedTypeForDiamond(GrExpression diamondNew) {
    ...
    if (pParent instanceof GrAssignmentExpression assignment && PsiTreeUtil.isAncestor(assignment.getRValue(), diamondNew, false)) {
      GrExpression lValue = assignment.getLValue();
      if (PsiUtil.mightBeLValue(lValue)) {
        return lValue.getNominalType();
      }
    }
    else if (pParent instanceof GrVariable variable && variable.getInitializerGroovy() == diamondNew) {
      return variable.getDeclaredType();
    }
    else if (pParent instanceof GrListOrMap) {
      if (PsiUtil.skipParentheses(pParent.getParent(), true) instanceof GrAssignmentExpression assignment
          && PsiTreeUtil.isAncestor(assignment.getRValue(), pParent, false)) {
        PsiElement lValue = PsiUtil.skipParentheses(assignment.getLValue(), false);
        if (lValue instanceof GrTupleExpression tupleExpr) {
          ...
          GrExpression[] expressions = tupleExpr.getExpressions();
  ...
  @Nullable
  public static PsiType normalizeWildcardTypeByPosition(@Nonnull PsiType type, @Nonnull GrExpression expression) {
    GrExpression topLevel = expression;
    while (topLevel.getParent() instanceof GrIndexProperty indexProp && indexProp.getInvokedExpression() == topLevel) {
      topLevel = indexProp;
    }

    PsiType normalized = doNormalizeWildcardByPosition(type, expression, topLevel);
    if (normalized instanceof PsiClassType && !PsiUtil.isAccessedForWriting(topLevel)) {
      return com.intellij.java.language.psi.util.PsiUtil.captureToplevelWildcards(normalized, expression);
    }
  ...
  @Nullable
  private static PsiType doNormalizeWildcardByPosition(PsiType type, GrExpression expression, GrExpression topLevel) {
  ...
  public static boolean hasElementType(@Nullable PsiElement next, @Nonnull IElementType type) {
    if (next == null) {
      return false;
    }
  ...
  public static boolean hasElementType(@Nullable PsiElement next, TokenSet set) {
    if (next == null) {
      return false;
    }
    ASTNode astNode = next.getNode();
  ...
  @RequiredReadAction
  public static PsiElement findTailingSemicolon(@Nonnull GrStatement statement) {
    PsiElement nextNonSpace = PsiUtil.skipWhitespaces(statement.getNextSibling(), true);
  ...
  public static PsiType inferReturnType(PsiElement position) {
    ...
    PsiElement parent = flowOwner.getContext();
  ...
  public static void insertPlaceHolderToModifierListAtEndIfNeeded(GrModifierList modifierList) {
    ...
    if (newLineAfterModifierList != null) {
      ...
      PsiElement newLineUpdated = findNewLineAfterElement(modifierList);
  ...
  @Nullable
  public static GrAnnotation getAnnotation(@Nonnull GrAnnotationNameValuePair pair) {
  ...
  public static <T extends PsiElement> T findElementInRange(PsiFile file,
    ...
    T element = ReflectionUtil.isAssignable(klass, commonParent.getClass())
      ? (T)commonParent
  ...
  @RequiredReadAction
  private static boolean checkRanges(@Nonnull PsiElement element, int startOffset, int endOffset) {
  ...
  public static boolean isSpreadAssignment(@Nullable GrExpression lValue) {
    if (lValue instanceof GrReferenceExpression) {
      ...
      if (dot != null && dot.getNode().getElementType() == GroovyTokenTypes.mSPREAD_DOT) {
        ...
        if (qualifier != null) {
          return isSpreadAssignment(qualifier);
        }
      }
    }
    return false;
  ...
  @RequiredWriteAction
  public static void replaceExpression(@Nonnull String newExpression,
                                       @Nonnull GrExpression expression) throws IncorrectOperationException {
  ...
  @RequiredWriteAction
  public static GrStatement replaceStatement(@Nonnull String newStatement,
                                             @Nonnull GrStatement statement) throws IncorrectOperationException {
  ...
  @Nullable
  public static PsiType getQualifierType(@Nonnull GrReferenceExpression ref) {
    GrExpression rtQualifier = getRuntimeQualifier(ref);
    if (rtQualifier != null) {
      return rtQualifier.getType();
    }

    PsiClass containingClass = null;
    GrMember member = PsiTreeUtil.getParentOfType(ref, GrMember.class);
    if (member == null) {
  ...
  @Nonnull
  public static GroovyResolveResult reflectedToBase(GroovyResolveResult result,
                                                    GrMethod baseMethod,
                                                    GrReflectedMethod reflectedMethod) {
    ...
    return new GroovyResolveResultImpl(baseMethod,
