...
import com.intellij.java.language.impl.psi.impl.light.JavaIdentifier;
...
public class PsiUtil {
  ...
  public static final Set<String> OPERATOR_METHOD_NAMES = Set.of("plus",
                                                                                   "minus",
                                                                                   "multiply",
                                                                                   "power",
                                                                                   "div",
  ...
  @Nullable
  public static PsiElement findModifierInList(@Nonnull GrModifierList list,
																							@GrModifier.GrModifierConstant @Nonnull String modifier) {
    for (PsiElement element : list.getModifiers()) {
      if (modifier.equals(element.getText())) {
        return element;
      }
    }
    return null;
  ...
  public static boolean isLValue(GroovyPsiElement element) {
    if (element instanceof GrExpression) {
      ...
      return parent instanceof GrAssignmentExpression && PsiTreeUtil.isAncestor(((GrAssignmentExpression)
        parent).getLValue(), element, false);
    }
    return false;
  }
  ...
  public static boolean isApplicable(@Nullable PsiType[] argumentTypes,
    ...
    return isApplicableConcrete(argumentTypes, method, substitutor, place,
                                eraseParameterTypes) != GrClosureSignatureUtil.ApplicabilityResult.inapplicable;
  }
  ...
  public static GrClosureSignatureUtil.ApplicabilityResult isApplicableConcrete(@Nullable PsiType[] argumentTypes,
    ...
    GrClosureSignature signature = eraseParameterTypes ? GrClosureSignatureUtil
      .createSignatureWithErasedParameterTypes(method) : GrClosureSignatureUtil.createSignature(method,
                                                                                                substitutor);
    ...
    if (method.isConstructor()) {
      ...
      if (parameters.length == 0 && argumentTypes.length == 1) {
        return InheritanceUtil.isInheritor(argumentTypes[0], CommonClassNames.JAVA_UTIL_MAP) ?
          GrClosureSignatureUtil.ApplicabilityResult.applicable : GrClosureSignatureUtil
    ...
    GrClosureSignatureUtil.ApplicabilityResult result = GrClosureSignatureUtil.isSignatureApplicableConcrete
      (signature, argumentTypes, place);
    ...
    if (method instanceof GrBuilderMethod && !((GrBuilderMethod)method).hasObligatoryNamedArguments()) {
      ...
      if (parameters.length > 0 && parameters[0].getType() instanceof GrMapType &&
        ...
        return GrClosureSignatureUtil.isSignatureApplicableConcrete(GrClosureSignatureUtil.removeParam
          (signature, 0), argumentTypes, place);
      }
    }
    return GrClosureSignatureUtil.ApplicabilityResult.inapplicable;
  }
  ...
  public static boolean isApplicable(@Nullable PsiType[] argumentTypes, GrClosureType type, GroovyPsiElement context) {
    return isApplicableConcrete(argumentTypes, type, context) != GrClosureSignatureUtil.ApplicabilityResult
      .inapplicable;
  }
  ...
  public static PsiType[] getArgumentTypes(@Nullable PsiElement place,
    ...
    if (parent instanceof GrIndexProperty) {
      ...
      PsiType[] argTypes = getArgumentTypes(index.getNamedArguments(), index.getExpressionArguments(),
                                            index.getClosureArguments(), nullAsBottom, stopAt, byShape);
    ...
    if (parent instanceof GrCall) {
      ...
      if (argList == null) {
        return getArgumentTypes(GrNamedArgument.EMPTY_ARRAY, GrExpression.EMPTY_ARRAY,
                                GrClosableBlock.EMPTY_ARRAY, nullAsBottom, stopAt, byShape);
      }
      else {
        return getArgumentTypes(argList.getNamedArguments(), argList.getExpressionArguments(),
                                GrClosableBlock.EMPTY_ARRAY, nullAsBottom, stopAt, byShape);
      }
    }
  ...
  @Nullable
  public static PsiType[] getArgumentTypes(@Nonnull GrNamedArgument[] namedArgs,
                                           @Nonnull GrExpression[] expressions,
  ...
  public static boolean isValidReferenceName(@Nonnull String text) {
    GroovyLexer lexer = new GroovyLexer();
    lexer.start(text);
    return TokenSets.REFERENCE_NAMES_WITHOUT_NUMBERS.contains(lexer.getTokenType()) && lexer.getTokenEnd() == text
      .length();
  }

  public static boolean isAccessible(@Nonnull PsiElement place, @Nonnull PsiMember member) {
    ...
    if (PsiTreeUtil.getParentOfType(place, GrDocComment.class) != null) {
      return true;
    }

    return com.intellij.java.language.psi.util.PsiUtil.isAccessible(member, place, null);
  }
  ...
  public static void reformatCode(PsiElement element) {
    ...
    if (viewProvider instanceof MultiplePsiFilesPerDocumentFileViewProvider) {
      MultiplePsiFilesPerDocumentFileViewProvider multiProvider = (MultiplePsiFilesPerDocumentFileViewProvider)
        viewProvider;
    ...
    try {
      CodeStyleManager.getInstance(element.getProject()).reformatText(file, textRange.getStartOffset(),
  ...
  public static Iterable<PsiClass> iterateSupers(@Nonnull final PsiClass psiClass, final boolean includeSelf) {
    return new Iterable<PsiClass>() {
      @Override
      public Iterator<PsiClass> iterator() {
        return new Iterator<PsiClass>() {
          IntStack indices = new IntStack();
  ...
  public static boolean isInDummyFile(@Nonnull PsiElement context) {
  ...
  public static boolean mightBeLValue(@Nullable GrExpression expr) {
    if (expr instanceof GrParenthesizedExpression) {
      return mightBeLValue(((GrParenthesizedExpression)expr).getOperand());
    }
    ...
    if ((isThisOrSuperRef(expr)) && GroovyConfigUtils.getInstance().isVersionAtLeast(expr,
                                                                                     GroovyConfigUtils.GROOVY1_8)) {
      return true;
    }
    return false;
  }
  ...
  private static boolean isRawClosureCall(GrMethodCallExpression call,
                                          GroovyResolveResult result,
                                          GrClosureType returnType) {
    ...
    if (signature instanceof GrClosureSignature) {
      ...
      PsiType[] types = getArgumentTypes(call.getInvokedExpression(), true);
      Trinity<GrClosureSignature, GrClosureSignatureUtil.ArgInfo<PsiType>[],
        GrClosureSignatureUtil.ApplicabilityResult> resultTrinity = types != null ? GrClosureSignatureUtil
        .getApplicableSignature(signature, types, call) : null;
      _signature = resultTrinity != null ? resultTrinity.first : null;
    }
    if (_signature != null) {
      return isRawType(_signature.getReturnType(), TypesUtil.composeSubstitutors(_signature.getSubstitutor(),
  ...
  private static boolean isRawIndexPropertyAccess(GrIndexProperty expr) {
    ...
    if (qualifierType instanceof PsiClassType) {
      ...
      if (InheritanceUtil.isInheritor(qualifierType, CommonClassNames.JAVA_UTIL_LIST)) {
        return com.intellij.java.language.psi.util.PsiUtil.extractIterableTypeParameter(qualifierType, false) == null;
      }

      if (InheritanceUtil.isInheritor(qualifierType, CommonClassNames.JAVA_UTIL_MAP)) {
        return com.intellij.java.language.psi.util.PsiUtil.substituteTypeParameter(qualifierType,
                                                                                   CommonClassNames.JAVA_UTIL_MAP, 1, false) == null;
      }
      ...
      MethodResolverProcessor processor = new MethodResolverProcessor("getAt", expr, false, qualifierType,
                                                                      argTypes, PsiType.EMPTY_ARRAY);

      PsiClass qClass = resolveResult.getElement();
      ResolveState state = ResolveState.initial().put(PsiSubstitutor.KEY, PsiSubstitutor.EMPTY);
      if (qClass != null) {
  ...
  public static boolean isRawType(@Nullable PsiType type, @Nonnull PsiSubstitutor substitutor) {
  ...
  public static boolean isNewLine(PsiElement element) {
    ...
    return elementType == GroovyTokenTypes.mNLS || elementType == TokenType.WHITE_SPACE && element.getText()
																																																	.contains("\n");
  }

  @Nullable
  public static PsiElement getPrevNonSpace(@Nonnull PsiElement elem) {
  ...
  @Nonnull
  public static PsiIdentifier getJavaNameIdentifier(@Nonnull GrNamedElement namedElement) {
  ...
  public static boolean hasEnclosingInstanceInScope(@Nonnull PsiClass clazz,
                                                    @Nullable PsiElement scope,
                                                    boolean isSuperClassAccepted) {
    return findEnclosingInstanceClassInScope(clazz, scope, isSuperClassAccepted) != null;
  }

  public static PsiClass findEnclosingInstanceClassInScope(@Nonnull PsiClass clazz,
                                                           @Nullable PsiElement scope,
                                                           boolean isInheritorOfClazzAccepted) {
    ...
    while (place != null && place != clazz && !(place instanceof PsiFile && place.isPhysical())) {
      if (place instanceof PsiClass) {
        if (isInheritorOfClazzAccepted) {
          if (InheritanceUtil.isInheritorOrSelf((PsiClass)place, clazz, true)) {
            return (PsiClass)place;
          }
        }
        else {
          if (clazz.getManager().areElementsEquivalent(place, clazz)) {
            return (PsiClass)place;
          }
        }
      }
  ...
  @Nullable
  public static PsiType getSmartReturnType(@Nonnull PsiMethod method) {
  ...
  public static boolean isClosurePropertyGetter(PsiMethod method) {
    ...
    if (methodName.startsWith("get") && GroovyPropertyUtils.isGetterName(methodName)) { // exclude isXXX()
      ...
      if (!modifiers.hasModifierProperty(PsiModifier.STATIC) && !modifiers.hasModifierProperty(PsiModifier
                                                                                                 .PRIVATE) && !modifiers.hasModifierProperty(
        PsiModifier.PROTECTED) && method.getParameterList()
                                        .getParametersCount() == 0) {
        PsiType type = getSmartReturnType(method);
        if (type != null && (TypesUtil.isClassType(type, CommonClassNames.JAVA_LANG_OBJECT) || TypesUtil
          .isClassType(type, GroovyCommonClassNames.GROOVY_LANG_CLOSURE))) {
          return true;
        }
      }
    }
  ...
  @Nonnull
  public static GroovyResolveResult[] getConstructorCandidates(@Nonnull PsiElement place,
                                                               @Nonnull GroovyResolveResult classCandidate,
                                                               @Nullable PsiType[] argTypes) {
    PsiElement element = classCandidate.getElement();
  ...
  public static GrReferenceExpression qualifyMemberReference(GrReferenceExpression refExpr,
                                                             PsiMember member,
                                                             String name) {
    ...
    if (member.hasModifierProperty(PsiModifier.STATIC)) {
      ...
      PsiClass containingClass = PsiTreeUtil.getParentOfType(refExpr, PsiClass.class);
  ...
  public static GroovyResolveResult[] getConstructorCandidates(PsiClassType classType,
                                                               PsiType[] argTypes,
                                                               GroovyPsiElement context) {
    PsiClassType.ClassResolveResult resolveResult = classType.resolveGenerics();
    ...
    GroovyResolveResult grResult = resolveResult instanceof GroovyResolveResult ? (GroovyResolveResult)
      resolveResult : new GroovyResolveResultImpl(psiClass, context, null, substitutor, true, true);
  ...
  @Nonnull
  public static PsiElement skipParenthesesIfSensibly(@Nonnull PsiElement element, boolean up) {
  ...
  @Nonnull
  public static PsiClass getOriginalClass(@Nonnull PsiClass aClass) {
  ...
  private static final String[] visibilityModifiers = new String[]{
    PsiModifier.PRIVATE,
  ...
  public static void escalateVisibility(PsiMember owner, PsiElement place) {
    ...
    for (; index < visibilityModifiers.length && !isAccessible(place, owner); index++) {
      ...
      com.intellij.java.language.psi.util.PsiUtil.setModifierProperty(owner, modifier, true);
    }
  }

  public static int getArgumentIndex(@Nonnull GrCall call, @Nonnull PsiElement argument) {
   ...
   */
  public static GrExpression[] getAllArguments(@Nonnull GrCall call) {
    GrArgumentList argumentList = call.getArgumentList();
    if (argumentList == null) {
  ...
  public static GrNamedArgument[] getFirstMapNamedArguments(@Nonnull GrCall grCall) {
  ...
  public static boolean isExpressionStatement(@Nonnull PsiElement expr) {
    ...
    if (parent instanceof GrIfStatement && (expr == ((GrIfStatement)parent).getThenBranch() || expr == (
      (GrIfStatement)parent).getElseBranch())) {
      return true;
    }
  ...
  public static String getAnnoAttributeValue(@Nonnull PsiAnnotation annotation,
  ...
  public static boolean getAnnoAttributeValue(@Nonnull PsiAnnotation annotation,
                                              String attributeName,
  ...
  public static boolean isReturnStatement(@Nonnull PsiElement statement) {
    GrControlFlowOwner controlFlowOwner = ControlFlowUtils.findControlFlowOwner(statement);
    if (controlFlowOwner instanceof GrOpenBlock) {
      ...
      else if (controlFlowOwnerParent instanceof PsiMethod && ((PsiMethod)controlFlowOwnerParent).getReturnType
        () == PsiType.VOID) {
        return false;
      }
    }
  ...
  public static ResolveResult getAccessObjectClass(GrExpression expression) {
    if (isThisOrSuperRef(expression)) {
  ...
  public static boolean isReferenceWithoutQualifier(@Nullable PsiElement element, @Nonnull String name) {
  ...
  public static boolean isProperty(@Nonnull GrField field) {
  ...
  public static boolean isConstructorHasRequiredParameters(@Nonnull PsiMethod constructor) {
  ...
  public static List<GrImportStatement> getValidImportStatements(GroovyFile file) {
    List<GrImportStatement> oldImports = new ArrayList<GrImportStatement>();
  ...
  public static boolean isCompileStatic(PsiElement e) {
    ...
    return containingMember != null && GroovyPsiManager.getInstance(containingMember.getProject()).isCompileStatic
      (containingMember);
  }
  ...
  private static boolean isThisOrSuperRef(PsiElement expression, IElementType token, boolean superClassAccepted) {
    ...
    if (qualifier == null) {
      ...
      if (resolved instanceof PsiClass) {
        if (hasEnclosingInstanceInScope(((PsiClass)resolved), ref, superClassAccepted)) {
          return true;
        }
        if (superClassAccepted && GrTraitUtil.isTrait((PsiClass)resolved) && scopeClassImplementsTrait((
                                                                                                         (PsiClass)resolved), ref)) {
          return true;
        }
      }
      return false;
  ...
  public static boolean scopeClassImplementsTrait(@Nonnull final PsiClass trait, @Nonnull final PsiElement place) {
  ...
  public static boolean isThisOrSuperRef(@Nullable PsiElement qualifier) {
    return qualifier instanceof GrReferenceExpression && (isThisReference(qualifier) || isSuperReference
      (qualifier));
  }
  ...
  @Nullable
  public static GrConstructorInvocation getConstructorInvocation(@Nonnull GrMethod constructor) {
  ...
  public static boolean isDGMMethod(@Nullable PsiElement element) {
    ...
    PsiMethod method = element instanceof GrGdkMethod ? ((GrGdkMethod)element).getStaticMethod() :
      (PsiMethod)element;
  ...
  public static boolean isVoidMethodCall(@Nullable GrExpression expression) {
    if (expression instanceof GrMethodCall && PsiType.NULL.equals(expression.getType())) {
      ...
      PsiType[] args = getArgumentTypes(((GrMethodCall)expression).getInvokedExpression(), true);
  ...
  public static boolean isVoidMethod(@Nonnull PsiMethod method) {
  ...
  public static boolean isBlockReturnVoid(@Nonnull final GrCodeBlock block) {
    return LanguageCachedValueUtil.getCachedValue(block, new CachedValueProvider<Boolean>() {
      @Nullable
      @Override
  ...
  public static boolean checkPsiElementsAreEqual(PsiElement l, PsiElement r) {
    ...
    for (int i = 0; i < rChildren.length; i++) {
      if (!checkPsiElementsAreEqual(lChildren[i], rChildren[i])) {
        return false;
      }
    }
    return true;
  ...
  public static PsiElement getPreviousNonWhitespaceToken(PsiElement e) {
    ...
    while (next != null && next.getNode().getElementType() == TokenType.WHITE_SPACE)
      next = PsiTreeUtil.prevLeaf(next);
    return next;
  }
}
