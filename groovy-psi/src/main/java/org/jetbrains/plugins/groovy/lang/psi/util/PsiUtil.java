...
public class PsiUtil
{
	...
	public static final Set<String> OPERATOR_METHOD_NAMES = ContainerUtil.newHashSet("plus", "minus", "multiply",
			"power", "div", "mod", "or", "and", "xor", "next", "previous", "getAt", "putAt", "leftShift",
			"rightShift", "isCase", "bitwiseNegate", "negative", "positive", "call");
	...
	public static GrClosureSignatureUtil.ApplicabilityResult isApplicableConcrete(@javax.annotation.Nullable PsiType[] argumentTypes,
			PsiMethod method,
			PsiSubstitutor substitutor,
			PsiElement place,
			final boolean eraseParameterTypes)
	...
	public static boolean isAccessible(@Nonnull PsiElement place, @Nonnull PsiMember member)
	{
		...
		if(place instanceof GrReferenceExpression && ((GrReferenceExpression) place).getQualifierExpression() == null)
		{
			...
			{ //calling top level script members from the same script file
				return true;
			}
		}
			...
			GrClosureType returnType)
	{
		...
		else
		{
			...
			final Trinity<GrClosureSignature, GrClosureSignatureUtil.ArgInfo<PsiType>[],
	...
	private static boolean isRawIndexPropertyAccess(GrIndexProperty expr)
	{
		...
		if(qualifierType instanceof PsiClassType)
		{
			...
			{
				return com.intellij.psi.util.PsiUtil.substituteTypeParameter(qualifierType,
	...
	@Nullable
	public static PsiElement getPrevNonSpace(@Nonnull final PsiElement elem)
	{
	...
	public static boolean isClosurePropertyGetter(PsiMethod method)
	{
		...
		{ // exclude isXXX()
			...
			if(!modifiers.hasModifierProperty(PsiModifier.STATIC) && !modifiers.hasModifierProperty(PsiModifier
					.PRIVATE) && !modifiers.hasModifierProperty(PsiModifier.PROTECTED) && method.getParameterList()
			...
			GroovyPsiElement context)
	{
		...
		final GroovyResolveResult grResult = resolveResult instanceof GroovyResolveResult ? (GroovyResolveResult)
	...
	@javax.annotation.Nullable
	public static PsiElement skipParentheses(@javax.annotation.Nullable PsiElement element, boolean up)
	{
		if(element == null)
		{
			return null;
	...
	public static boolean isSuperReference(@javax.annotation.Nullable PsiElement expression)
	{
	...
	public static boolean isDGMMethod(@Nullable PsiElement element)
	{
		...
		final PsiMethod method = element instanceof GrGdkMethod ? ((GrGdkMethod) element).getStaticMethod() :
				(PsiMethod) element;
