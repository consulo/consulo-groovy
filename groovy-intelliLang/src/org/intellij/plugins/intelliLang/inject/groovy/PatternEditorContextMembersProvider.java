...
public class PatternEditorContextMembersProvider extends NonCodeMembersContributor
{
	...
	public void processDynamicElements(
			@NotNull PsiType qualifierType, PsiScopeProcessor processor, PsiElement place, ResolveState state)
	{
	...
	private static CachedValueProvider<List<PsiElement>> createPatternProvider(final BaseInjection injection, final PsiFile file)
	{
		return new CachedValueProvider<List<PsiElement>>()
		{
			...
			{
				return new Result<List<PsiElement>>(Collections.<PsiElement>singletonList(getRootByClasses(InjectorUtils.getPatternClasses(injection
						.getSupportId()), file.getProject())), ArrayUtil.EMPTY_OBJECT_ARRAY);
			}
		};
	}
		...
		if("place".equals(name) && "injection".equals(parentTagName) && parentTag != null)
		{
			return new CachedValueProvider<List<PsiElement>>()
			{
				...
				public Result<List<PsiElement>> compute()
				{
					...
					return new Result<List<PsiElement>>(getRootsByClassNames(file, parentTag.getAttributeValue("injector-id")),
	...
	private static List<PsiElement> getRootsByClassNames(PsiFile file, String type)
	{
		...
		if(beanClass != null)
		{
			final GlobalSearchScope scope = GlobalSearchScope.getScopeRestrictedByFileTypes(GlobalSearchScope.allScope(project), XmlFileType.INSTANCE);
