...
public class ReplaceAbstractClassInstanceByMapIntention extends Intention {
    ...
    public LocalizeValue getText() {
        return GroovyIntentionLocalize.replaceAbstractClassInstanceByMapIntentionName();
    }
    ...
    protected void processIntention(@Nonnull PsiElement psiElement, Project project, Editor editor) throws IncorrectOperationException {
        ...
        if (methods.size() == 1) {
            Pair<PsiMethod, GrOpenBlock> pair = methods.get(0);
            ...
            for (Pair<PsiMethod, GrOpenBlock> pair : methods) {
                ...
                GrOpenBlock block = pair.getSecond();
    ...
    private static String createName(
        ...
        if (name == null) {
            name = GroovyNameSuggestionUtil.suggestVariableNameByType(
                type,
                new DefaultGroovyVariableNameValidator(context, generatedNames)
            )[0];
            assert name != null;
        }
