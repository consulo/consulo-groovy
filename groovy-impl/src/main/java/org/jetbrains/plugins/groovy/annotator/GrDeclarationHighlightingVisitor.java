...
class GrDeclarationHighlightingVisitor extends GroovyRecursiveElementVisitor
{
	...
	{
		assert myInfos != null;
		HighlightInfo info = HighlightInfo.newHighlightInfo(HighlightInfoType.INFORMATION).range(nameElement)
				.needsUpdateOnTyping(false).textAttributes(attribute).create();
		if(info != null)
		{
			myInfos.add(info);
		}
