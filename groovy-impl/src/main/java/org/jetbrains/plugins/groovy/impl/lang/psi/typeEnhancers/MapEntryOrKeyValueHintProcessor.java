...
public class MapEntryOrKeyValueHintProcessor extends SignatureHintProcessor {
    ...
    @Nonnull
    @Override
    public List<PsiType[]> inferExpectedSignatures(
        ...
        if (!InheritanceUtil.isInheritor(substituted, CommonClassNames.JAVA_UTIL_MAP)) {
            return List.of();
        }
        ...
        PsiClass mapEntry =
            JavaPsiFacade.getInstance(method.getProject()).findClass(CommonClassNames.JAVA_UTIL_MAP_ENTRY, method.getResolveScope());
        if (mapEntry == null) {
            return List.of();
        }
    ...
    private static boolean extractIndex(String[] options) {
        ...
        if (options.length == 1) {
            return StringUtil.parseBoolean(options[0], false);
        }
    ...
    private static Boolean parseIndex(String value) {
        ...
        Boolean parsedValue = StringUtil.parseBoolean(pair.getSecond(), false);
    ...
    private static Integer parseArgNum(String value) {
        ...
        Integer parsedValue = StringUtil.parseInt(pair.getSecond(), 0);
