...
import org.jetbrains.plugins.groovy.impl.refactoring.extract.method.ExtractMethodInfoHelper;
...
public class ExtractUtil {
  ...
  private static GrStatement[] createResultStatement(ExtractInfoHelper helper) {
    ...
    if (mustAdd.isEmpty()) {
      ...
      return new GrVariableDeclaration[]{
        factory.createVariableDeclaration(ArrayUtil.EMPTY_STRING_ARRAY, callExpression,
  ...
  private static List<GrStatement> generateVarDeclarations(List<VariableInfo> varInfos,
    ...
    if (distinctDeclaration) {
      for (VariableInfo info : varInfos) {
        result.add(factory.createVariableDeclaration(ArrayUtil.EMPTY_STRING_ARRAY, "", info.getType(),
                                                     info.getName()));
      }
    }
    else {
      ...
      result.add(factory.createVariableDeclaration(ArrayUtil.EMPTY_STRING_ARRAY, initializer,
                                                   varInfos.get(0).getType(), names));
    }
    return result;
  }
  ...
  private static void removeOldStatements(GrStatementOwner owner,
                                          ExtractInfoHelper helper) throws IncorrectOperationException {
  ...
  public static void generateBody(ExtractInfoHelper helper, boolean isVoid, StringBuilder buffer,
    ...
    if (!isSingleExpression(helper.getStatements()) && stringPartInfo == null) {
      ...
      GrExpression expr = stringPartInfo != null ? stringPartInfo.createLiteralFromSelected() : (GrExpression)
        PsiUtil.skipParentheses(helper.getStatements()[0], false);
  ...
  public static String[] getParameterString(ExtractInfoHelper helper, boolean useCanonicalText) {
    ...
    for (ParameterInfo info : infos) {
      if (info.passAsParameter()) {
        ...
        if (paramType == null || paramType.equalsToText(CommonClassNames.JAVA_LANG_OBJECT) || paramType.equals
          (PsiType.NULL)) {
          paramTypeText = "";
        }
        else {
  ...
  public static boolean isSingleExpression(GrStatement[] statements) {
    return statements.length == 1 && statements[0] instanceof GrExpression &&
      !(statements[0].getParent() instanceof GrVariableDeclarationOwner && statements[0] instanceof
