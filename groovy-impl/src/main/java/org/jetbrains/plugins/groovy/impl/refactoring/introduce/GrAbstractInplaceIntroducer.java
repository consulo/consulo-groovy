 ...
 */
public abstract class GrAbstractInplaceIntroducer<Settings extends GrIntroduceSettings> extends
  AbstractInplaceIntroducer<GrVariable, PsiElement> {
  ...
  public GrExpression restoreExpression(PsiFile containingFile,
    ...
    if (expression instanceof GrReferenceExpression && !(expression.getParent() instanceof GrMethodCall)) {
      ...
      if (((GrReferenceExpression)expression).resolve() == variable ||
        ...
        return (GrExpression)expression.replace(GroovyPsiElementFactory.getInstance(myProject)
                                                                       .createExpressionFromText(exprText, variable));
      }
    }
    if (expression == null) {
  ...
  protected PsiElement[] restoreOccurrences() {
    List<PsiElement> result = ContainerUtil.map(getOccurrenceMarkers(),
                                                marker -> PsiImplUtil.findElementInRange(myFile,
    ...
    SmartPsiElementPointer<GrVariable> pointer = ApplicationManager.getApplication().runWriteAction(new
                                                                                                      Computable<SmartPsiElementPointer<GrVariable>>() {
                                                                                                        ...
                                                                                                        public SmartPsiElementPointer<GrVariable> compute() {
                                                                                                          GrVariable var = runRefactoring(
                                                                                                            myContext,
                                                                                                            settings,
                                                                                                            false);
                                                                                                          return var != null ? SmartPointerManager
