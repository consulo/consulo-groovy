...
public class GroovyBlockGenerator {
  ...
  private static final TokenSet NESTED = TokenSet.create(GroovyElementTypes.REFERENCE_EXPRESSION,
                                                         GroovyElementTypes.PATH_INDEX_PROPERTY, GroovyElementTypes.PATH_METHOD_CALL,
  ...
  static List<ASTNode> getClosureBodyVisibleChildren(ASTNode node) {
    ...
    if (!children.isEmpty()) {
      ...
      if (first.getElementType() == GroovyTokenTypes.mLCURLY) {
        children.remove(0);
      }
    }
    ...
    if (!children.isEmpty()) {
      ...
      if (last.getElementType() == GroovyTokenTypes.mRCURLY) {
  ...
  public List<Block> generateSubBlocks() {
    ...
    if ((elementType == GroovyTokenTypes.mSTRING_LITERAL || elementType == GroovyTokenTypes.mGSTRING_LITERAL) &&
      ...
      if (text.length() > 6) {
        if (text.substring(0, 3).equals("'''") && text.substring(text.length() - 3).equals("'''") || text
          .substring(0, 3).equals("\"\"\"") & text.substring(text.length() - 3).equals("\"\"\"")) {
    ...
    if (elementType == GroovyElementTypes.GSTRING || elementType == GroovyElementTypes.REGEX || elementType ==
      GroovyTokenTypes.mREGEX_LITERAL || elementType == GroovyTokenTypes.mDOLLAR_SLASH_REGEX_LITERAL) {
      FormattingContext context = myNode.getPsi() instanceof GrString && ((GrString)myNode.getPsi())
        .isPlainString() ? myContext.createContext(true) : myContext;
      ...
      for (ASTNode childNode : children) {
        if (childNode.getTextRange().getLength() > 0) {
          subBlocks.add(new GroovyBlock(childNode, getIndent(childNode), Wrap.createWrap(WrapType.NONE,
                                                                                         false), context));
        }
      }
      return subBlocks;
    ...
    if (NESTED.contains(elementType) && blockPsi.getParent() != null && !NESTED.contains(blockPsi.getParent()
                                                                                                 .getNode().getElementType())) {
      List<Block> subBlocks = new ArrayList<Block>();
      AlignmentProvider.Aligner dotsAligner = myContext.getSettings().ALIGN_MULTILINE_CHAINED_METHODS ?
        myAlignmentProvider.createAligner(false) : null;

      Wrap wrap = myWrappingProcessor.getChainedMethodCallWrap();
      addNestedChildren(myNode.getPsi(), subBlocks, dotsAligner, true, wrap);
      return subBlocks;
    }

    if (blockPsi instanceof GrListOrMap && ((GrListOrMap)blockPsi).isMap() && myContext.getGroovySettings()
      .ALIGN_NAMED_ARGS_IN_MAP) {
    ...
    if (isListLikeClause(blockPsi)) {
      ...
      if (mustAlign(blockPsi, astNodes)) {
        AlignmentProvider.Aligner aligner = myAlignmentProvider.createAligner(false);
    ...
    Indent indent = GroovyIndentProcessor.getChildIndent(myBlock, parameterListNode);
        GroovyBlock block = new GroovyBlock(parameterListNode, indent, myWrap, mySettings, myGroovySettings,
        myAlignmentProvider);
        blocks.add(block);
      }
...

      {
        ...
        ClosureBodyBlock bodyBlock = new ClosureBodyBlock(parameterListNode, indent,
                                                          Wrap.createWrap(WrapType.NONE, false), myContext);
    ...
    if (blockPsi instanceof GrCodeBlock || blockPsi instanceof GroovyFile || classLevel) {
      return generateSubBlockForCodeBlocks(classLevel, visibleChildren(myNode),
                                           myContext.getGroovySettings().INDENT_LABEL_BLOCKS);
    }
    ...
    if (blockPsi instanceof GrMethod) {
      ...
      for (ASTNode childNode : getGroovyChildren(myNode)) {
        ...
        if (childNode.getElementType() == GroovyElementTypes.PARAMETERS_LIST) {
          subBlocks.add(new ParameterListBlock(((GrMethod)blockPsi), Indent.getNoneIndent(),
                                               Wrap.createWrap(WrapType.NONE, false), myContext));
        }
        else if (canBeCorrectBlock(childNode)) {
          subBlocks.add(new GroovyBlock(childNode, getIndent(childNode), getChildWrap(childNode),
                                        myContext));
        }
      }
      return subBlocks;
      ...
      if (myContext.getSettings().ALIGN_MULTILINE_FOR) {
        ...
        AlignmentProvider.Aligner parenthesesAligner = myAlignmentProvider.createAligner(false);
      ...
      if (myContext.getSettings().ALIGN_MULTILINE_TERNARY_OPERATION) {
        ...
        AlignmentProvider.Aligner exprAligner = myAlignmentProvider.createAligner(false);
    ...
    if (indentLabelBlocks && isCodeBlock()) {
      ...
      for (int i = 0; i < flattenChildren.size(); i++) {
        ...
        if (childNode.getElementType() == GroovyElementTypes.LABELED_STATEMENT) {
          ...
          subBlocks.add(new GrLabelBlock(childNode, flattenChildren.subList(start + 1, i), classLevel,
                                         getIndent(childNode), getChildWrap(childNode), myContext));
          i--;
        }
        else {
          subBlocks.add(new GroovyBlock(childNode, getIndent(childNode), getChildWrap(childNode),
  ...
  private boolean shouldSkip(boolean classLevel, PsiElement psi) {
    if (psi instanceof PsiComment) {
      ...
      if (prev != null && prev.getNode().getElementType() != GroovyTokenTypes.mNLS || classLevel &&
        !fieldGroupEnded(psi)) {
        return true;
      }
    }
  ...
  private void alignVariableDeclarations(List<GrStatement> group, boolean classLevel) {
    ...
    for (GrStatement statement : group) {
      ...
      if (classLevel && myContext.getSettings().ALIGN_GROUP_FIELD_DECLARATIONS) {
        ...
        ASTNode current_eq = variables[variables.length - 1].getNode().findChildByType(GroovyTokenTypes
                                                                                         .mASSIGN);
        if (current_eq != null) {
  ...
  private void alignSpockTable(List<GrStatement> group) {
    ...
    List<AlignmentProvider.Aligner> alignments = ContainerUtil.map2List(getSpockTable(first),
                                                                        leaf -> myAlignmentProvider.createAligner(leaf,
                                                                                                                  true,
  ...
  private static boolean isTablePart(PsiElement psi) {
    return psi instanceof GrBinaryExpression && (GroovyTokenTypes.mBOR == ((GrBinaryExpression)psi)
      .getOperationTokenType() || GroovyTokenTypes.mLOR == ((GrBinaryExpression)psi).getOperationTokenType
      ());
  }
  ...
  private boolean mustAlign(PsiElement blockPsi, List<ASTNode> children) {
    // We don't want to align single call argument if it's a closure. The reason is that it looks better to have
    // call like
    //
    // foo({
    //   println 'xxx'
    ...
    if (blockPsi instanceof GrArgumentList && myContext.getSettings().ALIGN_MULTILINE_PARAMETERS_IN_CALLS) {
      return !(children.size() == 3 &&
        ...
        (children.get(1).getElementType() == GroovyElementTypes.CLOSABLE_BLOCK || children.get(1)
                                                                                          .getElementType() == GroovyElementTypes.LIST_OR_MAP) &&
        children.get(2).getElementType() == GroovyTokenTypes.mRPAREN);
    }

    if (blockPsi instanceof GrAssignmentExpression && ((GrAssignmentExpression)blockPsi).getRValue() instanceof
  ...
  private static boolean isKeyword(ASTNode node) {
    ...
    return TokenSets.KEYWORDS.contains(node.getElementType()) || TokenSets.BRACES.contains(node.getElementType())
      && !PlatformPatterns.psiElement().withText(")").withParent(GrArgumentList.class).afterLeaf("," +
                                                                                                   "").accepts(node.getPsi());
  }
  ...
  private List<Block> generateForMultiLineString() {
    ArrayList<Block> subBlocks = new ArrayList<Block>();
    int start = myNode.getTextRange().getStartOffset();
    int end = myNode.getTextRange().getEndOffset();

    subBlocks.add(new GroovyBlockWithRange(myNode, Indent.getNoneIndent(), new TextRange(start, start + 3),
                                           Wrap.createWrap(WrapType.NONE, false), myContext));
    subBlocks.add(new GroovyBlockWithRange(myNode,
                                           Indent.getAbsoluteNoneIndent(),
                                           new TextRange(start + 3,
                                                         end - 3),
                                           Wrap.createWrap(WrapType.NONE, false),
                                           myContext));
    subBlocks.add(new GroovyBlockWithRange(myNode, Indent.getAbsoluteNoneIndent(), new TextRange(end - 3, end),
                                           Wrap.createWrap(WrapType.NONE, false), myContext));
    return subBlocks;
  }
  ...
  private List<Block> generateForBinaryExpr() {
    ArrayList<Block> subBlocks = new ArrayList<Block>();
    AlignmentProvider.Aligner alignment = myContext.getSettings().ALIGN_MULTILINE_BINARY_OPERATION ?
      myAlignmentProvider.createAligner(false) : null;
  ...
  private void addNestedChildren(PsiElement elem,
                                 List<Block> list,
                                 @Nullable AlignmentProvider.Aligner aligner,
                                 boolean topLevel,
                                 Wrap wrap) {
    List<ASTNode> children = visibleChildren(elem.getNode());
    if (elem instanceof GrMethodCallExpression) {
      ...
      if (invokedExpression instanceof GrQualifiedReference) {
        PsiElement nameElement = ((GrQualifiedReference)invokedExpression).getReferenceNameElement();
        if (nameElement != null) {
          ...
          if (i < grandChildren.size()) {
            ...
            list.add(new MethodCallWithoutQualifierBlock(nameElement, wrap, topLevel, children, elem,
                                                         myContext));
          }
          return;
        }
  ...
  private static boolean isAfterMultiLineClosure(ASTNode dot) {
    ...
    if (prev != null) {
      if (prev instanceof GrMethodCall) {
        PsiElement last = prev.getLastChild();
  ...
  void addNestedChildrenSuffix(List<Block> list,
    ...
    for (int i = 1; i < limit; i++) {
      ...
      if (canBeCorrectBlock(childNode)) {
        ...
        Indent indent = topLevel || NESTED.contains(type) || type == GroovyTokenTypes.mIDENT || TokenSets.DOTS
          .contains(type) && !isAfterMultiLineClosure(childNode) ? Indent
          .getContinuationWithoutFirstIndent() : Indent.getNoneIndent();
