...
public class GroovyOverrideImplementExploreUtil
{
			...
			boolean skipImplemented)
	{
		...
		for(HierarchicalMethodSignature signature : allMethodSignatures)
		{
			...
			if(method instanceof GrTraitMethod)
			{
				...
				{
					processMethod(aClass, skipImplemented, abstracts, finals, concretes, resolveHelper,
							superSignature, superSignature.getMethod());
				}
			}
			else
			{
		...
		else
		{
			for(Map.Entry<MethodSignature, PsiMethod> entry : concretes.entrySet())
			{
				...
				if(finals.get(signature) == null)
				{
					...
					if(abstractOne == null || !abstractOne.getContainingClass().isInheritor(concrete
							.getContainingClass(), true) ||
							CommonClassNames.JAVA_LANG_OBJECT.equals(concrete.getContainingClass().getQualifiedName()))
					{
						PsiSubstitutor subst = OverrideImplementExploreUtil.correctSubstitutor(concrete,
			...
			PsiMethod method)
	{
		...
		if(method.isConstructor() && (!aClass.isInheritor(hisClass, false) || aClass instanceof PsiAnonymousClass ||
				aClass.isEnum()))
		{
			return;
		}
			...
			Map<MethodSignature, CandidateInfo> result)
	{
		for(Map.Entry<MethodSignature, PsiMethod> entry : abstracts.entrySet())
		{
			...
			if(concrete == null || PsiUtil.getAccessLevel(concrete.getModifierList()) < PsiUtil.getAccessLevel
					(abstractOne.getModifierList()) || !abstractOne.getContainingClass().isInterface() && abstractOne
					.getContainingClass().isInheritor(concrete.getContainingClass(),
							true) || isDefaultMethod(abstractOne))
			{
				if(finals.get(signature) == null)
				{
					PsiSubstitutor subst = OverrideImplementExploreUtil.correctSubstitutor(abstractOne,
							signature.getSubstitutor());
		...
		for(final PsiMethod method : new GroovyMethodImplementor().getMethodsToImplement(aClass))
		{
			MethodSignature signature = MethodSignatureUtil.createMethodSignature(method.getName(),
					method.getParameterList(), method.getTypeParameterList(), PsiSubstitutor.EMPTY,
