...
public class GrMethodConflictUtil {
    ...
    private static void checkForClosurePropertySignatureOverload(
        PsiClass clazz,
        GrMethod prototype,
            ...
            public void visitClosureSignature(GrClosureSignature signature) {
                ...
                for (MethodSignature prototypeSignature : prototypeSignatures) {
                    ...
                    for (int i = 0; i < types.length; i++) {
                        if (!TypesUtil.isAssignableByMethodCallConversion(
                            types[i],
                            params[i].getType(),
                            refactoredMethod.getParameterList()
                        )) {
                    ...
                    conflicts.putValue(
                        getter,
                        GroovyRefactoringLocalize.refactoredMethodWillCoverClosureProperty(
                            name,
                            RefactoringUIUtil.getDescription(getter.getContainingClass(), false)
                        )
                    );
                }
            }
        ...
        for (MethodSignature prototypeSignature : prototypeSignatures) {
            for (PsiMethod method : signatures.get(prototypeSignature)) {
                if (method != refactoredMethod) {
                    ...
                    conflicts.putValue(
                        method,
                        GroovyRefactoringLocalize.methodDuplicate(signaturePresentation, RefactoringUIUtil.getDescription(clazz, false))
                    );
                    break;
                }
            }
    ...
    private static void checkForAccessorOverloading(
        PsiClass clazz,
        GrMethod prototype,
        MultiMap<PsiElement, LocalizeValue> conflicts
    ) {
        if (GroovyPropertyUtils.isSimplePropertySetter(prototype)) {
            ...
            if (setter instanceof GrAccessorMethod) {
                conflicts.putValue(setter, GroovyRefactoringLocalize.replaceSetterForProperty(propertyName));
            }
        }
            ...
            if (getter instanceof GrAccessorMethod) {
                conflicts.putValue(getter, GroovyRefactoringLocalize.replaceGetterForProperty(propertyName));
            }
        }
    }
}
