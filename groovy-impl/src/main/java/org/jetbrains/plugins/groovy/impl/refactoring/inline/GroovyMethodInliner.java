...
public class GroovyMethodInliner implements InlineHandler.Inliner {
    ...
    public MultiMap<PsiElement, LocalizeValue> getConflicts(@Nonnull PsiReference reference, @Nonnull PsiElement referenced) {
        ...
        if (!(element instanceof GrExpression && element.getParent() instanceof GrCallExpression)) {
            MultiMap<PsiElement, LocalizeValue> map = new MultiMap<>();
            map.putValue(element, GroovyRefactoringLocalize.cannotInlineReference0(element.getText()));
            return map;
        }
    ...
    @Nonnull
    private static MultiMap<PsiElement, LocalizeValue> collectConflicts(
        @Nonnull GrCallExpression call,
        @Nonnull Collection<GroovyInlineMethodUtil.ReferenceExpressionInfo> infos
    ) {
        ...
        for (GroovyInlineMethodUtil.ReferenceExpressionInfo info : infos) {
            if (!PsiUtil.isAccessible(call, info.declaration)) {
                if (info.declaration instanceof PsiMethod) {
                    ...
                    conflicts.putValue(info.declaration, GroovyRefactoringLocalize.methodIsNotAccessibleFormContext0(name));
                }
                    ...
                    if (!(info.declaration instanceof GrField && ((GrField) info.declaration).getGetters().length > 0)) { // conflict if field doesn't have implicit getters
                        ...
                        conflicts.putValue(info.declaration, GroovyRefactoringLocalize.fieldIsNotAccessibleFormContext0(name));
                    }
                }
            }
            ...
            if (visitor.containsSupers()) {
                conflicts.putValue(info.expression, GroovyRefactoringLocalize.superReferenceIsUsed());
            }
        }
    ...
    public void inlineUsage(@Nonnull UsageInfo usage, @Nonnull PsiElement referenced) {
        ...
        Editor editor = getCurrentEditorIfApplicable(element);

        GrCallExpression call = (GrCallExpression) element.getParent();
        RangeMarker marker =
            inlineReferenceImpl(call, myMethod, isOnExpressionOrReturnPlace(call), GroovyInlineMethodUtil.isTailMethodCall(call), editor);
    ...
    @Nullable
    static RangeMarker inlineReferenceImpl(
        @Nonnull GrCallExpression call,
        ...
        try {
            ...
            if (resultOfCallExplicitlyUsed && !isTailMethodCall) {
                ...
                if (PsiType.VOID.equals(methodType)) {
                    ...
                    GrExpression returnExpression = ControlFlowUtils.extractReturnExpression(returnStatements.iterator().next());
            ...
            if (!isTailMethodCall && resultOfCallExplicitlyUsed && returnCount == 1) {
                ...
                for (GrStatement returnStatement : returnStatements) {
                    if (returnStatement instanceof GrReturnStatement) {
                        GrExpression returnValue = ((GrReturnStatement) returnStatement).getReturnValue();
            ...
            if (resultOfCallExplicitlyUsed && !isTailMethodCall) {
                ...
                RangeMarker marker =
                    editor != null ? editor.getDocument().createRangeMarker(range.getStartOffset(), range.getEndOffset(), true) : null;
                reformatOwner(owner);
                return marker;
            }
            else {
    ...
    @Nonnull
    private static String generateQualifierName(
        @Nonnull GrCallExpression call,
        @Nullable GrMethod method,
     ...
     */
    @Nonnull
    private static GrMethod prepareNewMethod(
        @Nonnull GrCallExpression call,
        @Nonnull GrMethod method,
            ...
            if (name != null) {
                ...
                if (!newName.equals(namedElement.getName())) {
                    Collection<PsiReference> refs =
                        ReferencesSearch.search(namedElement, GlobalSearchScope.projectScope(namedElement.getProject()), false).findAll();
