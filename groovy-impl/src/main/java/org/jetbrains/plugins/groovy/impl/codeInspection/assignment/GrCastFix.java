...
public class GrCastFix extends GroovyFix implements LocalQuickFix {
    ...
    private static GrExpression findExpressionToCast(ProblemDescriptor descriptor) {
        ...
        LOG.error(
            "can't find expression to cast at position " + element.getTextRange(),
            AttachmentFactory.get().create(url, file.getText())
        );
        return null;
    }
    ...
    public LocalizeValue getName() {
        return LocalizeValue.localizeTODO("Cast to " + myExpectedType.getPresentableText());
    }
}
