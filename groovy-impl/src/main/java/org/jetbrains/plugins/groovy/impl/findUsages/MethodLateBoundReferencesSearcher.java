...
public class MethodLateBoundReferencesSearcher extends QueryExecutorBase<PsiReference, MethodReferencesSearch.SearchParameters>
    ...
    private static SearchScope getUseScope(PsiMethod method) {
        ...
        if (file != null && scope instanceof GlobalSearchScope globalSearchScope) {
            ...
            if (vfile != null && ProjectRootManager.getInstance(project).getFileIndex().isInSource(vfile)) {
                return new GrSourceFilterScope(globalSearchScope);
            }
        }
        return scope;
    }

    private static void orderSearching(SearchScope searchScope, String name, @Nonnull SearchRequestCollector collector, int paramCount) {
        ...
        collector.searchWord(name, searchScope, UsageSearchContext.IN_CODE, true, new RequestResultProcessor("groovy.lateBound") {
            ...
            public boolean processTextOccurrence(
                ...
                if (parent instanceof GrMethodCall methodCall) {
                    if (!argumentsMatch(methodCall, paramCount)) {
                        return true;
                    }
                }
