...
public class GroovyNameSuggestionUtil
{
	...
	public static final Logger LOG = Logger.getInstance("org.jetbrains.plugins.groovy.refactoring" +
	...
	private static void generateVariableNameByTypeInner(PsiType type,
	...
	private static void generateNameByString(Set<String> possibleNames,
			...
			Project project)
	{
		if(!PsiNameHelper.getInstance(project).isIdentifier(value))
		{
			return;
		}
	...
	private static void generateByType(PsiType type, Set<String> possibleNames, NameValidator validator)
	{
		...
		{
			possibleNames.add(validator.validateName(GroovyNamesUtil.deleteNonLetterFromString(typeName.toLowerCase())
					, true));
		}
	...
	private static void generateNamesForCollectionType(PsiType type, Set<String> possibleNames,
			NameValidator validator)
	{
	...
	private static PsiType getCollectionComponentType(PsiType type, Project project)
	{
		...
		@SuppressWarnings({"ConstantConditions"}) PsiClass collectionClass = facade.findClass("java.util.Collection",
				type.getResolveScope());
		if(collectionClass == null || collectionClass.getTypeParameters().length != 1)
		{
			return null;
		}
		PsiSubstitutor substitutor = TypeConversionUtil.getClassSubstitutor(collectionClass, clazz,
