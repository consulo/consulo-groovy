...
public class GroovyAnnotator extends GroovyElementVisitor
{
	...
	public void visitTypeArgumentList(GrTypeArgumentList typeArgumentList)
	{
		...
		{
			myHolder.createWarningAnnotation(typeArgumentList, GroovyBundle.message("type.argument.list.is.not.allowed" +
					".here"));
			return;
		}
		...
		{
			myHolder.createWarningAnnotation(typeArgumentList, GroovyBundle.message("wrong.number.of.type.arguments",
		...
		for(int i = 0; i < parameters.length; i++)
		{
			...
			for(PsiClassType superType : superTypes)
			{
				...
				{
					myHolder.createWarningAnnotation(arguments[i], GroovyBundle.message("type.argument.0.is.not.in.its" +
							".bound.should.extend.1", argType.getCanonicalText(), superType.getCanonicalText()));
					break;
				}
			}
		}
	...
	public void visitNamedArgument(GrNamedArgument argument)
	{
		...
		if(parent instanceof GrArgumentList)
		{
			...
			{
				myHolder.createErrorAnnotation(argument, GroovyBundle.message("named.arguments.are.not.allowed.inside" +
						".index.operations"));
			}
		}
	}
	...
	private void checkForCommandExpressionSyntax(GrMethodCall methodCall)
	{
		...
		{
			myHolder.createErrorAnnotation(methodCall, GroovyBundle.message("is.not.supported.in.version",
					groovyConfig.getSDKVersion(methodCall)));
		}
	}
	...
	public void visitTryStatement(GrTryCatchStatement statement)
	{
		...
		for(GrCatchClause clause : clauses)
		{
			...
			PsiType type = typeElement != null ? typeElement.getType() : TypesUtil.createType(CommonClassNames
					.JAVA_LANG_EXCEPTION, statement);
				...
				for(int i = 0; i < types.length; i++)
				{
					if(checkExceptionUsed(usedExceptions, parameter, elements[i], types[i]))
					{
						...
						for(int j = 0; j < types.length; j++)
						{
							...
							{
								myHolder.createWarningAnnotation(elements[i], GroovyBundle.message("unnecessary.type",
										types[i].getCanonicalText(), types[j].getCanonicalText())).registerFix(new
										GrRemoveExceptionFix(true));
							}
						}
					}
				}
	...
	public void visitCatchClause(GrCatchClause clause)
	{
		...
		if(typeElement != null)
		{
			...
			{
				myHolder.createErrorAnnotation(typeElement, GroovyBundle.message("catch.statement.parameter.type" +
						".should.be.a.subclass.of.throwable"));
			}
		}
	}
	...
	public void visitDocComment(GrDocComment comment)
	{
		...
		if(!text.endsWith("*/"))
		{
			...
			myHolder.createErrorAnnotation(new TextRange(range.getEndOffset() - 1, range.getEndOffset()),
	...
	public void visitVariableDeclaration(GrVariableDeclaration variableDeclaration)
	{
		if(variableDeclaration.isTuple())
		{
			...
			if(last != null)
			{
				...
				{
					myHolder.createErrorAnnotation(list, GroovyBundle.message("tuple.declaration.should.end.with.def" +
							".modifier"));
				}
			}
			else
	...
	private boolean checkExceptionUsed(List<PsiType> usedExceptions,
		...
		for(PsiType exception : usedExceptions)
		{
			if(exception.isAssignableFrom(type))
			{
				myHolder.createWarningAnnotation(typeElement != null ? typeElement : parameter.getNameIdentifierGroovy
						(), GroovyBundle.message("exception.0.has.already.been.caught",
						type.getCanonicalText())).registerFix(new GrRemoveExceptionFix(parameter.getTypeElementGroovy
	...
	private void checkFinalParameterAccess(GrReferenceExpression ref)
	{
		...
		if(resolved instanceof GrParameter)
		{
			...
			if(parameter.isPhysical() && parameter.hasModifierProperty(PsiModifier.FINAL) && PsiUtil.isLValue(ref))
			{
				...
				{
					myHolder.createErrorAnnotation(ref, GroovyBundle.message("cannot.assign.a.value.to.final.parameter" +
							".0", parameter.getName()));
				}
			}
		}
	...
	private void checkFinalFieldAccess(@Nonnull GrReferenceExpression ref)
	{
		...
		if(resolved instanceof GrField && resolved.isPhysical() && ((GrField) resolved).hasModifierProperty
				(PsiModifier.FINAL) && PsiUtil.isLValue(ref))
		{
			...
			if(containingClass != null && PsiTreeUtil.isAncestor(containingClass, ref, true))
			{
				...
				{
					if(container instanceof GrMethod && ((GrMethod) container).isConstructor() || container instanceof
							GrClassInitializer && !((GrClassInitializer) container).isStatic())
					{
						return;
					}
				}

				myHolder.createErrorAnnotation(ref, GroovyBundle.message("cannot.assign.a.value.to.final.field.0",
						field.getName()));
			}
		}
	}
	...
	private void checkStringNameIdentifier(GrReferenceExpression ref)
	{
		...
		else if(elementType == GroovyTokenTypes.mREGEX_LITERAL || elementType == GroovyTokenTypes
	...
	public void visitTypeDefinition(GrTypeDefinition typeDefinition)
	{
				...
				typeDefinition instanceof GrTypeParameter))
		{
			...
			final Annotation errorAnnotation = myHolder.createErrorAnnotation(range,
					GroovyBundle.message("class.definition.is.not.expected.here"));
			...
			if(collection.size() > 1 && !sameAccessModifier(collection))
			{
				for(GrMethod method : collection)
				{
					holder.createErrorAnnotation(GrHighlightUtil.getMethodHeaderTextRange(method),
							GroovyBundle.message("mixing.private.and.public.protected.methods.of.the.same.name"));
				}
			}
		}
	}
	...
	private static void checkAnnotationCollector(AnnotationHolder holder, GrTypeDefinition definition)
	{
		...
		{
			holder.createErrorAnnotation(definition.getNameIdentifierGroovy(), GroovyBundle.message("annotation" +
					".collector.cannot.have.attributes"));
		}
	}
	...
	private static void checkConstructors(AnnotationHolder holder, GrTypeDefinition typeDefinition)
	{
		...
		{
			if(!hasImplicitDefConstructor && (defConstructor == null || !PsiUtil.isAccessible(typeDefinition,
					defConstructor)))
			{
				final TextRange range = GrHighlightUtil.getClassHeaderTextRange(typeDefinition);
				holder.createErrorAnnotation(range, GroovyBundle.message("there.is.no.default.constructor.available.in" +
						".class.0", qName)).registerFix(QuickFixFactory.getInstance()
						.createCreateConstructorMatchingSuperFix(typeDefinition));
			}
			return;
		}
	...
	public void visitEnumConstant(GrEnumConstant enumConstant)
	{
		...
		if(argumentList != null && PsiImplUtil.hasNamedArguments(argumentList) && !PsiImplUtil.hasExpressionArguments
				(argumentList))
		{
			...
			if(constructor != null)
			{
				...
				{
					myHolder.createErrorAnnotation(argumentList, GroovyBundle.message("the.usage.of.a.map.entry" +
							".expression.to.initialize.an.enum.is.currently.not.supported"));
				}
			}
		}
	}
	...
	private static void checkRecursiveConstructors(AnnotationHolder holder, PsiMethod[] constructors)
	{
		...
		for(PsiMethod constructor : constructors)
		{
			...
			for(constructor = nodes.get(constructor); constructor != null && current.add(constructor); constructor =
					nodes.get(constructor))
			{
	...
	{
		if(expression.getOperationTokenType() == GroovyTokenTypes.mINC || expression.getOperationTokenType() ==
				GroovyTokenTypes.mDEC)
		{
			...
			if(operand instanceof GrReferenceExpression && ((GrReferenceExpression) operand).getQualifier() == null)
			{
				...
				if(trait != null)
				{
					...
					if(resolved instanceof GrField && ((GrField) resolved).getContainingClass() instanceof
							GrTraitTypeDefinition)
					{
						myHolder.createErrorAnnotation(expression, GroovyBundle.message("0.expressions.on.trait.fields" +
								".properties.are.not.supported.in.traits", expression.getOperationToken().getText()));
					}
				}
			}
		}
	...
	public void visitOpenBlock(GrOpenBlock block)
	{
		if(block.getParent() instanceof GrMethod)
		{
			...
			if(method.getModifierList().hasExplicitModifier(PsiModifier.ABSTRACT) || GrTraitUtil.isInterface(method
					.getContainingClass()))
			{
				final Annotation annotation = myHolder.createErrorAnnotation(block, GroovyBundle.message("abstract" +
						".methods.must.not.have.body"));
	...
	public void visitMethod(GrMethod method)
	{
		...
		{
			myHolder.createWeakWarningAnnotation(nameIdentifier, GroovyBundle.message("method.0.is.too.complex.too" +
					".analyze", method.getName()));
		}
		...
		if(method.isConstructor())
		{
			...
			{
				myHolder.createErrorAnnotation(nameIdentifier, GroovyBundle.message("constructors.are.not.allowed.in" +
						".anonymous.class"));
			}
		...
		if(method.getBlock() == null && !method.hasModifierProperty(PsiModifier.NATIVE) && !GrTraitUtil
				.isMethodAbstract(method))
		{
			final Annotation annotation = myHolder.createErrorAnnotation(nameIdentifier,
					GroovyBundle.message("not.abstract.method.should.have.body"));
	...
	private static void checkGetterOfImmutable(AnnotationHolder holder, GrMethod method)
	{
		...
		holder.createErrorAnnotation(method.getNameIdentifierGroovy(), GroovyBundle.message("repetitive.method.name" +
				".0", method.getName()));
	}
	...
	private static void checkConstructorOfImmutableClass(AnnotationHolder holder, GrMethod method)
	{
		...
		holder.createErrorAnnotation(method.getNameIdentifierGroovy(), GroovyBundle.message("explicit.constructors.are" +
				".not.allowed.in.immutable.class"));
	}

	private static void checkOverridingMethod(@Nonnull AnnotationHolder holder, @Nonnull GrMethod method)
	{
		final List<HierarchicalMethodSignature> signatures = method.getHierarchicalMethodSignature()
				.getSuperSignatures();
		...
		for(HierarchicalMethodSignature signature : signatures)
		{
			...
			if(superMethod.hasModifierProperty(PsiModifier.FINAL))
			{
				...
				final String current = GroovyPresentationUtil.getSignaturePresentation(method.getSignature
				...
				holder.createErrorAnnotation(GrHighlightUtil.getMethodHeaderTextRange(method),
						GroovyBundle.message("method.0.cannot.override.method.1.in.2.overridden.method.is.final",
								current, superPresentation, superQName));
			...
			if(PsiModifier.PUBLIC.equals(superModifier) && (PsiModifier.PROTECTED.equals(currentModifier) ||
					PsiModifier.PRIVATE.equals(currentModifier)) || PsiModifier.PROTECTED.equals(superModifier) &&
					PsiModifier.PRIVATE.equals(currentModifier))
			{
				final String currentPresentation = GroovyPresentationUtil.getSignaturePresentation(method.getSignature
						(PsiSubstitutor.EMPTY));
				final String superPresentation = GroovyPresentationUtil.getSignaturePresentation(signature);
				final String superQName = getQNameOfMember(superMethod);

				final PsiElement modifier = PsiUtil.findModifierInList(method.getModifierList(), currentModifier);
				holder.createErrorAnnotation(modifier != null ? modifier : method.getNameIdentifierGroovy(),
						GroovyBundle.message("method.0.cannot.have.weaker.access.privileges.1.than.2.in.3.4",
								currentPresentation, currentModifier, superPresentation, superQName, superModifier));
			}
		}
	}
	...
	private static void checkMethodWithTypeParamsShouldHaveReturnType(AnnotationHolder holder, GrMethod method)
	{
		...
		if(parameterList != null)
		{
			...
			if(typeElement == null)
			{
				...
				final TextRange range = new TextRange(parameterListTextRange.getEndOffset(),
						parameterListTextRange.getEndOffset() + 1);
				holder.createErrorAnnotation(range, GroovyBundle.message("method.with.type.parameters.should.have" +
						".return.type"));
			}
		}
	}
	...
	private static void checkOptionalParametersInAbstractMethod(AnnotationHolder holder, GrMethod method)
	{
		...
		for(GrParameter parameter : method.getParameters())
		{
			...
			if(initializerGroovy != null)
			{
				...
				TextRange textRange = new TextRange(assignOperator.getTextRange().getEndOffset(),
						initializerGroovy.getTextRange().getEndOffset());
				holder.createErrorAnnotation(textRange, GroovyBundle.message("default.initializers.are.not.allowed.in" +
						".abstract.method"));
			}
		}
	}
	...
	public void visitVariable(GrVariable variable)
	{
		...
		if(parent instanceof GrForInClause)
		{
			...
			if(delimiter.getNode().getElementType() == GroovyTokenTypes.mCOLON)
			{
				...
				{
					Annotation annotation = myHolder.createErrorAnnotation(variable.getNameIdentifierGroovy(),
							GroovyBundle.message("java.style.for.each.statement.requires.a.type.declaration"));
		...
		if(duplicate instanceof GrVariable && (variable instanceof GrField || ResolveUtil.isScriptField(variable) || !
				(duplicate instanceof GrField)))
		{
			final String key = duplicate instanceof GrField ? "field.already.defined" : "variable.already.defined";
			myHolder.createErrorAnnotation(variable.getNameIdentifierGroovy(), GroovyBundle.message(key,
					variable.getName()));
		}
	...
	private void checkName(GrVariable variable)
	{
		...
		myHolder.createErrorAnnotation(variable.getNameIdentifierGroovy(), GroovyBundle.message("incorrect.variable" +
				".name"));
	}
	...
	public void visitReturnStatement(GrReturnStatement returnStatement)
	{
		...
		if(value != null)
		{
			...
			if(type != null)
			{
				final GrParameterListOwner owner = PsiTreeUtil.getParentOfType(returnStatement, GrMethod.class,
				...
				if(owner instanceof PsiMethod)
				{
					...
					else
					{
						...
						if(methodType != null)
						{
							...
							{
								myHolder.createErrorAnnotation(value, GroovyBundle.message("cannot.return.from.void" +
										".method"));
							}
						}
					}
	...
	public void visitListOrMap(GrListOrMap listOrMap)
	{
		...
		if(constructorReference instanceof LiteralConstructorReference && ((LiteralConstructorReference)
				constructorReference).getConstructedClassType() != null)
		{
			...
			{
				myHolder.createInfoAnnotation(startToken, null).setTextAttributes(GroovySyntaxHighlighter
						.LITERAL_CONVERSION);
			}
			...
			{
				myHolder.createInfoAnnotation(endToken, null).setTextAttributes(GroovySyntaxHighlighter
						.LITERAL_CONVERSION);
			}
		}
		...
		{
			myHolder.createErrorAnnotation(listOrMap, GroovyBundle.message("collection.literal.contains.named.argument" +
	...
	public void visitCodeReferenceElement(GrCodeReferenceElement refElement)
	{
		...
		if(resolved instanceof PsiClass && (((PsiClass) resolved).isAnnotationType() || GrAnnotationCollector
				.findAnnotationCollector((PsiClass) resolved) != null && refElement.getParent() instanceof
				GrAnnotation))
		{
	...
	private static void checkClassInitializerModifiers(AnnotationHolder holder, GrModifierList modifierList)
	{
		...
		for(@GrModifier.GrModifierConstant String modifier : GrModifier.GROOVY_MODIFIERS)
		{
			...
			checkModifierIsNotAllowed(modifierList, modifier, GroovyBundle.message("initializer.cannot.be.0",
					modifier), holder);
		}
	}
	...
	private static void checkFieldModifiers(AnnotationHolder holder, GrVariableDeclaration fieldDeclaration)
	{
		...
		if(modifierList.hasExplicitModifier(PsiModifier.VOLATILE) && modifierList.hasExplicitModifier(PsiModifier
				.FINAL))
		{
			final Annotation annotation = holder.createErrorAnnotation(modifierList,
					GroovyBundle.message("illegal.combination.of.modifiers.volatile.and.final"));
			registerFix(annotation, new GrModifierFix(member, PsiModifier.VOLATILE, true, false,
					GrModifierFix.MODIFIER_LIST), modifierList);
			registerFix(annotation, new GrModifierFix(member, PsiModifier.FINAL, true, false,
					GrModifierFix.MODIFIER_LIST), modifierList);
		}

		checkModifierIsNotAllowed(modifierList, PsiModifier.NATIVE, GroovyBundle.message("variable.cannot.be.native"),
				holder);
		checkModifierIsNotAllowed(modifierList, PsiModifier.ABSTRACT, GroovyBundle.message("variable.cannot.be" +
				".abstract"), holder);

		if(member.getContainingClass() instanceof GrInterfaceDefinition)
		{
			checkModifierIsNotAllowed(modifierList, PsiModifier.PRIVATE, GroovyBundle.message("interface.members.are" +
					".not.allowed.to.be", PsiModifier.PRIVATE), holder);
			checkModifierIsNotAllowed(modifierList, PsiModifier.PROTECTED, GroovyBundle.message("interface.members.are" +
					".not.allowed.to.be", PsiModifier.PROTECTED), holder);
		}
	}
	...
	private static void checkModifierIsNotAllowed(@Nonnull GrModifierList modifierList,
	...
	private static void checkModifierIsNotAllowedImpl(@Nonnull GrModifierList modifierList,
			@Nonnull @GrModifier.GrModifierConstant String modifier,
			...
			final boolean explicit)
	{
		if(explicit ? modifierList.hasModifierProperty(modifier) : modifierList.hasExplicitModifier(modifier))
		{
			...
			registerFix(annotation, new GrModifierFix((PsiMember) modifierList.getParent(), modifier, true, false,
					GrModifierFix.MODIFIER_LIST), modifierList);
		}
	}
	...
	private static void checkAnnotationAttributeType(GrTypeElement element, AnnotationHolder holder)
	{
		...
		else if(element instanceof GrClassTypeElement)
		{
			...
			{
				if(CommonClassNames.JAVA_LANG_STRING.equals(((PsiClass) resolved).getQualifiedName()))
				{
					return;
				}
				if(CommonClassNames.JAVA_LANG_CLASS.equals(((PsiClass) resolved).getQualifiedName()))
				{
					return;
				}
	...
	static void checkMethodReturnType(PsiMethod method, PsiElement toHighlight, AnnotationHolder holder)
	{
		...
		for(HierarchicalMethodSignature superMethodSignature : superSignatures)
		{
			...
			if(superReturnType == PsiType.VOID && method instanceof GrMethod && ((GrMethod) method).getReturnTypeElementGroovy() == null)
			{
				return;
			}
	...
	@javax.annotation.Nullable
	private static String checkSuperMethodSignature(@Nonnull PsiMethod superMethod,
			...
			@Nonnull PsiType returnType)
	{
		...
		final String presentation = returnType.getCanonicalText() + " " + GroovyPresentationUtil
				.getSignaturePresentation(methodSignature);
		final String basePresentation = superReturnType.getCanonicalText() + " " + GroovyPresentationUtil
				.getSignaturePresentation(superMethodSignature);
		return GroovyBundle.message("return.type.is.incompatible", presentation, qName, basePresentation, baseQName);
	}

	@Nonnull
	private static PsiType substituteSuperReturnType(@Nonnull MethodSignatureBackedByPsiMethod superMethodSignature,
			@Nonnull MethodSignatureBackedByPsiMethod methodSignature,
			@Nonnull PsiType superReturnType)
	{
		...
		{ //see 8.4.5
			PsiSubstitutor unifyingSubstitutor = MethodSignatureUtil.getSuperMethodSignatureSubstitutor
					(methodSignature, superMethodSignature);
			substitutedSuperReturnType = unifyingSubstitutor == null ? superReturnType : unifyingSubstitutor
					.substitute(superMethodSignature.getSubstitutor().substitute(superReturnType));
		}
		else
		{
	...
	@Nonnull
	private static String getQName(@javax.annotation.Nullable PsiClass aClass)
	{
		if(aClass instanceof PsiAnonymousClass)
		{
			return GroovyBundle.message("anonymous.class.derived.from.0", ((PsiAnonymousClass) aClass)
					.getBaseClassType().getCanonicalText());
		}
		if(aClass != null)
		{
	...
	public void visitWildcardTypeArgument(GrWildcardTypeArgument wildcardTypeArgument)
	{
		...
		checkTypeArgForPrimitive(wildcardTypeArgument.getBoundTypeElement(), GroovyBundle.message("primitive.bound" +
	...
	private void highlightNamedArgs(GrNamedArgument[] namedArguments)
	{
		for(GrNamedArgument namedArgument : namedArguments)
		{
			...
			if(label != null && label.getExpression() == null && label.getNameElement().getNode().getElementType() !=
	...
	public void visitNewExpression(GrNewExpression newExpression)
	{
		...
		{
			myHolder.createErrorAnnotation(constructorTypeArguments, GroovyBundle.message("groovy.does.not.support" +
		...
		if(element instanceof PsiClass)
		{
			...
			if(clazz.hasModifierProperty(PsiModifier.ABSTRACT))
			{
				...
				{
					String message = clazz.isInterface() ? GroovyBundle.message("cannot.instantiate.interface",
							clazz.getName()) : GroovyBundle.message("cannot.instantiate.abstract.class",
							clazz.getName());
			...
			if(newExpression.getQualifier() != null)
			{
				...
				{
					myHolder.createErrorAnnotation(newExpression, GroovyBundle.message("qualified.new.of.static" +
							".class"));
				}
			}
		}
	...
	private static boolean checkDiamonds(GrCodeReferenceElement refElement, AnnotationHolder holder)
	{
		...
		{
			final String message = GroovyBundle.message("diamonds.are.not.allowed.in.groovy.0",
					configUtils.getSDKVersion(refElement));
	...
	public void visitClosure(GrClosableBlock closure)
	{
		...
		if(TypeInferenceHelper.isTooComplexTooAnalyze(closure))
		{
			...
			{
				Document document = PsiDocumentManager.getInstance(closure.getProject()).getDocument(closure
						.getContainingFile());
				if(document == null)
				{
					return;
			...
			myHolder.createWeakWarningAnnotation(new TextRange(startOffset, endOffset),
					GroovyBundle.message("closure.is.too.complex.to.analyze"));
		}
	}
	...
	private static boolean followsError(GrClosableBlock closure)
	{
		...
		return prev instanceof PsiErrorElement || prev instanceof PsiWhiteSpace && prev.getPrevSibling() instanceof
				PsiErrorElement;
	}
	...
	private void checkRegexLiteral(PsiElement regex)
	{
		...
		if("$/".equals(quote))
		{
			...
			{
				myHolder.createErrorAnnotation(regex, GroovyBundle.message("dollar.slash.strings.are.not.allowed.in" +
						".0", config.getSDKVersion(regex)));
			}
		}
		...
		{
			if(!GrStringUtil.parseRegexCharacters(part, new StringBuilder(part.length()), null,
					regex.getText().startsWith("/")))
			{
				myHolder.createErrorAnnotation(regex, GroovyBundle.message("illegal.escape.character.in.string" +
						".literal"));
				return;
			}
		}
		...
		if("/".equals(quote))
		{
			if(!config.isVersionAtLeast(regex, GroovyConfigUtils.GROOVY1_8))
			{
				if(text.contains("\n") || text.contains("\r"))
				{
					myHolder.createErrorAnnotation(regex, GroovyBundle.message("multiline.slashy.strings.are.not" +
							".allowed.in.groovy.0", config.getSDKVersion(regex)));
					return;
				}
			}
		}
	...
	public void visitGStringExpression(GrString gstring)
	{
		for(GrStringContent part : gstring.getContents())
		{
			...
			{
				myHolder.createErrorAnnotation(part, GroovyBundle.message("illegal.escape.character.in.string" +
						".literal"));
				return;
			}
		}
	...
	public void visitGStringInjection(GrStringInjection injection)
	{
		if(((GrString) injection.getParent()).isPlainString())
		{
			...
			{
				myHolder.createErrorAnnotation(injection, GroovyBundle.message("injection.should.not.contain.line" +
						".feeds"));
			}
		}
	}
	...
	private void checkStringLiteral(PsiElement literal)
	{
		...
		{
			myHolder.createErrorAnnotation(literal, GroovyBundle.message("illegal.escape.character.in.string" +
					".literal"));
			return;
		}
	...
	public void visitForInClause(GrForInClause forInClause)
	{
		...
		for(PsiElement modifier : modifiers)
		{
			...
			myHolder.createErrorAnnotation(modifier, GroovyBundle.message("not.allowed.modifier.in.forin",
					modifierText));
		}
	}
	...
	private boolean checkAnnotationAttributeValue(@Nullable GrAnnotationMemberValue value,
			@Nonnull PsiElement toHighlight)
	{
		...
		myHolder.createErrorAnnotation(toHighlight, GroovyBundle.message("expected.0.to.be.inline.constant",
				value.getText()));
		return true;
	}
	...
	{
		checkAnnotationList(myHolder, importStatement.getAnnotationList(), GroovyBundle.message("import.statement" +
	...
	public void visitExtendsClause(GrExtendsClause extendsClause)
	{
		...
		{
			myHolder.createErrorAnnotation(extendsClause, GroovyBundle.message("annotation.types.may.not.have.extends" +
					".clause"));
		}
		else if(typeDefinition.isTrait())
		{
			checkReferenceList(myHolder, extendsClause, IS_TRAIT, GroovyBundle.message("only.traits.expected.here"),
					null);
		}
		else if(typeDefinition.isInterface())
		{
			checkReferenceList(myHolder, extendsClause, IS_INTERFACE, GroovyBundle.message("no.class.expected.here"),
					null);
		}
		...
		{
			checkReferenceList(myHolder, extendsClause, IS_NOT_INTERFACE, GroovyBundle.message("no.interface.expected" +
					".here"), new ChangeExtendsImplementsQuickFix(typeDefinition));
	...
	public void visitImplementsClause(GrImplementsClause implementsClause)
	{
		...
		if(typeDefinition.isAnnotationType())
		{
			myHolder.createErrorAnnotation(implementsClause, GroovyBundle.message("annotation.types.may.not.have" +
					".implements.clause"));
		}
		else if(GrTraitUtil.isInterface(typeDefinition))
		{
			myHolder.createErrorAnnotation(implementsClause, GroovyBundle.message("no.implements.clause.allowed.for" +
					".interface")).registerFix(new ChangeExtendsImplementsQuickFix(typeDefinition));
		}
		else
		{
			checkReferenceList(myHolder, implementsClause, IS_INTERFACE, GroovyBundle.message("no.class.expected" +
					".here"), new ChangeExtendsImplementsQuickFix(typeDefinition));
			checkForWildCards(myHolder, implementsClause);
		}
	}

	private static void checkReferenceList(@Nonnull AnnotationHolder holder,
			@Nonnull GrReferenceList list,
			@Nonnull Condition<PsiClass> applicabilityCondition,
	...
	private static void checkThisOrSuperReferenceExpression(final GrReferenceExpression ref, AnnotationHolder holder)
	{
		...
		if(qualifier instanceof GrReferenceExpression)
		{
			...
			if(resolved instanceof PsiClass)
			{
				GrTypeDefinition containingClass = PsiTreeUtil.getParentOfType(ref, GrTypeDefinition.class, true,
						GroovyFile.class);

				if(elementType == GroovyTokenTypes.kSUPER && containingClass != null && GrTraitUtil.isTrait((PsiClass)
						resolved))
				{
					...
					{
						holder.createInfoAnnotation(nameElement, null).setTextAttributes(GroovySyntaxHighlighter
								.KEYWORD);
						return; // reference to trait method
					}
				}

				if(containingClass == null || containingClass.getContainingClass() == null && !containingClass
						.isAnonymous())
				{
					holder.createErrorAnnotation(ref, GroovyBundle.message("qualified.0.is.allowed.only.in.nested.or" +
							".inner.classes", nameElement.getText()));
					return;
				}
	...
	private static void checkImplementedMethodsOfClass(AnnotationHolder holder, GrTypeDefinition typeDefinition)
	{
		...
		final TextRange range = GrHighlightUtil.getClassHeaderTextRange(typeDefinition);
		final Annotation annotation = holder.createErrorAnnotation(range, GroovyBundle.message("method.is.not" +
				".implemented", notImplementedMethodName));
		registerImplementsMethodsFix(typeDefinition, abstractMethod, annotation);
	}

	private static void registerImplementsMethodsFix(@Nonnull GrTypeDefinition typeDefinition,
			...
			@Nonnull Annotation annotation)
	{
		...
		if(!JavaPsiFacade.getInstance(typeDefinition.getProject()).getResolveHelper().isAccessible(abstractMethod,
				typeDefinition, null))
		{
			registerFix(annotation, new GrModifierFix(abstractMethod, PsiModifier.PUBLIC, true, true,
					GrModifierFix.MODIFIER_LIST_OWNER), abstractMethod);
			registerFix(annotation, new GrModifierFix(abstractMethod, PsiModifier.PROTECTED, true, true,
					GrModifierFix.MODIFIER_LIST_OWNER), abstractMethod);
		}

		if(!(typeDefinition instanceof GrAnnotationTypeDefinition) && typeDefinition.getModifierList() != null)
		{
			registerFix(annotation, new GrModifierFix(typeDefinition, PsiModifier.ABSTRACT, false, true,
					GrModifierFix.MODIFIER_LIST_OWNER), typeDefinition);
		}
	}
	...
	private static void checkInnerMethod(AnnotationHolder holder, GrMethod grMethod)
	{
		...
		{
			holder.createErrorAnnotation(grMethod.getNameIdentifierGroovy(), GroovyBundle.message("Inner.methods.are" +
					".not.supported"));
		}
	}

	private static void registerMakeAbstractMethodNotAbstractFix(Annotation annotation,
			GrMethod method,
			boolean makeClassAbstract)
	{
		...
		registerFix(annotation, new GrModifierFix(method, PsiModifier.ABSTRACT, false, false,
				GrModifierFix.MODIFIER_LIST_OWNER), method);
		if(makeClassAbstract)
		{
			...
			if(containingClass != null)
			{
				...
				{
					registerFix(annotation, new GrModifierFix(containingClass, PsiModifier.ABSTRACT, false, true,
							GrModifierFix.MODIFIER_LIST_OWNER), containingClass);
				}
			}
		}
	}
	...
	private static void checkMethodDefinitionModifiers(AnnotationHolder holder, GrMethod method)
	{
		...
		checkModifierIsNotAllowed(modifiersList, PsiModifier.VOLATILE, GroovyBundle.message("method.has.incorrect" +
				".modifier.volatile"), holder);
		...
		if(method.getParent() instanceof GroovyFileBase)
		{
			if(isMethodAbstract)
			{
				final Annotation annotation = holder.createErrorAnnotation(getModifierOrList(modifiersList,
						PsiModifier.ABSTRACT), GroovyBundle.message("script.method.cannot.have.modifier.abstract"));
				registerMakeAbstractMethodNotAbstractFix(annotation, method, false);
			}

			checkModifierIsNotAllowed(modifiersList, PsiModifier.NATIVE, GroovyBundle.message("script.cannot.have" +
					".modifier.native"), holder);
		}
		...
		else if(method.getParent() != null && method.getParent().getParent() instanceof GrTypeDefinition)
		{
			...
			{
				checkModifierIsNotAllowed(modifiersList, PsiModifier.PROTECTED, GroovyBundle.message("trait.method" +
						".cannot.be.protected"), holder);
			}
			//interface
			else if(containingTypeDef.isInterface())
			{
				checkModifierIsNotAllowed(modifiersList, PsiModifier.STATIC, GroovyBundle.message("interface.must.have" +
						".no.static.method"), holder);
				checkModifierIsNotAllowed(modifiersList, PsiModifier.PRIVATE, GroovyBundle.message("interface.members" +
						".are.not.allowed.to.be", PsiModifier.PRIVATE), holder);
				checkModifierIsNotAllowed(modifiersList, PsiModifier.PROTECTED, GroovyBundle.message("interface" +
						".members.are.not.allowed.to.be", PsiModifier.PROTECTED), holder);
			}
			else if(containingTypeDef.isAnonymous())
			{
				...
				{
					final Annotation annotation = holder.createErrorAnnotation(getModifierOrList(modifiersList,
							PsiModifier.ABSTRACT), GroovyBundle.message("anonymous.class.cannot.have.abstract" +
							".method"));
					registerMakeAbstractMethodNotAbstractFix(annotation, method, false);
				}
			}
			...
			else
			{
				...
				{
					final Annotation annotation = holder.createErrorAnnotation(modifiersList,
							GroovyBundle.message("only.abstract.class.can.have.abstract.method"));
					registerMakeAbstractMethodNotAbstractFix(annotation, method, true);
				}
			}
			...
			if(method.isConstructor())
			{
				checkModifierIsNotAllowed(modifiersList, PsiModifier.STATIC, GroovyBundle.message("constructor.cannot" +
						".have.static.modifier"), holder);
			}
		}
		...
		{
			final Annotation annotation = holder.createErrorAnnotation(getModifierOrList(modifiersList,
					PsiModifier.NATIVE), GroovyBundle.message("native.methods.cannot.have.body"));
			registerFix(annotation, new GrModifierFix((PsiMember) modifiersList.getParent(), PsiModifier.NATIVE, true,
					false, GrModifierFix.MODIFIER_LIST), modifiersList);
			annotation.registerFix(QuickFixFactory.getInstance().createDeleteMethodBodyFix(method));
		}
	}

	private static void checkForAbstractAndFinalCombination(AnnotationHolder holder,
			GrMember member,
			GrModifierList modifiersList)
	{
		...
		{
			final Annotation annotation = holder.createErrorAnnotation(modifiersList,
					GroovyBundle.message("illegal.combination.of.modifiers.abstract.and.final"));
			registerFix(annotation, new GrModifierFix(member, PsiModifier.FINAL, false, false,
					GrModifierFix.MODIFIER_LIST), modifiersList);
			registerFix(annotation, new GrModifierFix(member, PsiModifier.ABSTRACT, false, false,
					GrModifierFix.MODIFIER_LIST), modifiersList);
		}
	}

	@Nonnull
	private static PsiElement getModifierOrList(@Nonnull GrModifierList modifiersList,
			@GrModifier.GrModifierConstant final String modifier)
	{
	...
	private static void checkOverrideAnnotation(AnnotationHolder holder, GrModifierList list, GrMethod method)
	{
		...
		try
		{
			MethodSignatureBackedByPsiMethod superMethod = SuperMethodsSearch.search(method, null, true,
					false).findFirst();
			if(superMethod == null)
			{
				holder.createWarningAnnotation(overrideAnnotation, GroovyBundle.message("method.doesnot.override" +
						".super"));
			}
		}
	...
	private static void checkTypeDefinitionModifiers(AnnotationHolder holder, GrTypeDefinition typeDefinition)
	{
		...
		for(PsiClassType classType : extendsListTypes)
		{
			...
			{
				final Annotation annotation = holder.createErrorAnnotation(typeDefinition.getNameIdentifierGroovy(),
						GroovyBundle.message("final.class.cannot.be.extended"));
				registerFix(annotation, new GrModifierFix(typeDefinition, PsiModifier.FINAL, false, false,
						GrModifierFix.MODIFIER_LIST_OWNER), typeDefinition);
			}
		}
		...
		checkModifierIsNotAllowed(modifiersList, PsiModifier.TRANSIENT, GroovyBundle.message("modifier.transient.not" +
				".allowed.here"), holder);
		checkModifierIsNotAllowed(modifiersList, PsiModifier.VOLATILE, GroovyBundle.message("modifier.volatile.not" +
				".allowed.here"), holder);

		/**** interface ****/
		if(typeDefinition.isInterface())
		{
			checkModifierIsNotAllowed(modifiersList, PsiModifier.FINAL, GroovyBundle.message("intarface.cannot.have" +
					".modifier.final"), holder);
		}
	}

	private static void checkDuplicateModifiers(AnnotationHolder holder, @Nonnull GrModifierList list,
			PsiMember member)
	{
		...
		for(PsiElement modifier : modifiers)
		{
			...
			{
				final Annotation annotation = holder.createErrorAnnotation(list, GroovyBundle.message("duplicate" +
						".modifier", name));
				registerFix(annotation, new GrModifierFix(member, name, false, false, GrModifierFix.MODIFIER_LIST),
						list);
			}
			else
			{
	...
	private static void checkAccessModifiers(AnnotationHolder holder,
			@Nonnull GrModifierList modifierList,
			PsiMember member)
	{
		...
		{
			final Annotation annotation = holder.createErrorAnnotation(modifierList,
					GroovyBundle.message("illegal.combination.of.modifiers"));
			if(hasPrivate)
			{
				registerFix(annotation, new GrModifierFix(member, PsiModifier.PRIVATE, false, false,
						GrModifierFix.MODIFIER_LIST), modifierList);
			}
			if(hasProtected)
			{
				registerFix(annotation, new GrModifierFix(member, PsiModifier.PROTECTED, false, false,
						GrModifierFix.MODIFIER_LIST), modifierList);
			}
			if(hasPublic)
			{
				registerFix(annotation, new GrModifierFix(member, PsiModifier.PUBLIC, false, false,
						GrModifierFix.MODIFIER_LIST), modifierList);
			}
		}
		...
		{
			checkModifierIsNotAllowed(modifierList, PsiModifier.PRIVATE, GroovyBundle.message("top.level.class.maynot" +
					".have.private.modifier"), holder);
			checkModifierIsNotAllowed(modifierList, PsiModifier.PROTECTED, GroovyBundle.message("top.level.class" +
					".maynot.have.protected.modifier"), holder);
		}
	}

	private static void checkDuplicateMethod(PsiClass clazz, AnnotationHolder holder)
	{
		MultiMap<MethodSignature, PsiMethod> map = GrClosureSignatureUtil.findRawMethodSignatures(clazz.getMethods(),
				clazz);
		...
		for(MethodSignature signature : map.keySet())
		{
			...
			if(methods.size() > 1)
			{
				...
				for(PsiMethod method : methods)
				{
					//noinspection ConstantConditions
					holder.createErrorAnnotation(GrHighlightUtil.getMethodHeaderTextRange(method),
							GroovyBundle.message("method.duplicate", signaturePresentation,
	...
	private static void checkTypeDefinition(AnnotationHolder holder, GrTypeDefinition typeDefinition)
	{
		...
		if(typeDefinition.isAnonymous())
		{
			...
			{
				holder.createErrorAnnotation(typeDefinition.getNameIdentifierGroovy(), GroovyBundle.message("anonymous" +
						".classes.are.not.supported", configUtils.getSDKVersion(typeDefinition)));
			}
			...
			{
				holder.createErrorAnnotation(typeDefinition.getNameIdentifierGroovy(), GroovyBundle.message("anonymous" +
						".classes.cannot.be.created.from.traits"));
			}
		}
		else if(typeDefinition.isTrait())
		{
			if(!configUtils.isVersionAtLeast(typeDefinition, GroovyConfigUtils.GROOVY2_3))
			{
				...
				assert keyword != null;
				holder.createErrorAnnotation(keyword, GroovyBundle.message("traits.are.not.supported.in.groovy.0",
						configUtils.getSDKVersion(typeDefinition)));
			}
		}
		else if(typeDefinition.getContainingClass() != null && !(typeDefinition instanceof GrEnumTypeDefinition))
		{
			if(!configUtils.isVersionAtLeast(typeDefinition, GroovyConfigUtils.GROOVY1_7))
			{
				holder.createErrorAnnotation(typeDefinition.getNameIdentifierGroovy(),
						GroovyBundle.message("inner.classes.are.not.supported", configUtils.getSDKVersion
								(typeDefinition)));
			}
		}
		...
		{
			holder.createErrorAnnotation(typeDefinition.getNameIdentifierGroovy(), GroovyBundle.message("annotation" +
					".type.cannot.be.inner"));
		}
	...
	private static void checkForWildCards(AnnotationHolder holder, @javax.annotation.Nullable GrReferenceList clause)
	{
		if(clause == null)
		{
			return;
		...
		for(GrCodeReferenceElement element : elements)
		{
			...
			if(list != null)
			{
				for(GrTypeElement type : list.getTypeArgumentElements())
				{
					...
					{
						holder.createErrorAnnotation(type, GroovyBundle.message("wildcards.are.not.allowed.in.extends" +
								".list"));
					}
				}
			}
	...
	private static void checkDuplicateClass(GrTypeDefinition typeDefinition, AnnotationHolder holder)
	{
		...
		if(containingClass != null)
		{
			...
			{
				holder.createErrorAnnotation(typeDefinition.getNameIdentifierGroovy(), GroovyBundle.message("duplicate" +
						".inner.class", name));
			}
		}
