...
public class GroovySmartCompletionContributor extends CompletionContributor
{
	...
	private static final ElementPattern<PsiElement> IN_CAST_PARENTHESES = PlatformPatterns.psiElement().withSuperParent(3, PlatformPatterns.psiElement(GrTypeCastExpression.class).withParent
	...
	public GroovySmartCompletionContributor()
	{
		...
		extend(CompletionType.SMART, IN_CAST_PARENTHESES, new CompletionProvider()
		{
			...
			public void addCompletions(@Nonnull CompletionParameters parameters, ProcessingContext context, @Nonnull CompletionResultSet result)
			{
				...
				final boolean overwrite = PlatformPatterns.psiElement().afterLeaf(PlatformPatterns.psiElement().withText("(").withParent(GrTypeCastExpression.class)).accepts(parameters
						.getOriginalPosition());
	...
	{
		return !com.intellij.psi.util.PsiUtil.isInnerClass(psiClass) || PsiUtil.hasEnclosingInstanceInScope(psiClass.getContainingClass(), identifierCopy, true);
	}
