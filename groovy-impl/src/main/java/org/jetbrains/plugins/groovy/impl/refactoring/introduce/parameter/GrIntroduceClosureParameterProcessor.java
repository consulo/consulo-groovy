...
public class GrIntroduceClosureParameterProcessor extends BaseRefactoringProcessor {
                ...
                for (UsageInfo usageInfo : usagesIn) {
                    if (!(usageInfo.getElement() instanceof PsiMethod) && !(usageInfo instanceof InternalUsageInfo)) {
                        if (!PsiTreeUtil.isAncestor(containingClass, usageInfo.getElement(), false)) {
                            conflicts.putValue(expression, RefactoringBundle.message(
                                "parameter.initializer.contains.0" +
    ...
    private static void changeSignature(GrClosableBlock block, GrIntroduceParameterSettings settings) {
        ...
        if (block.getArrow() == null) {
            final PsiElement arrow = block.addAfter(
    ...
    private static void processExternalUsage(
        UsageInfo usage,
        GrIntroduceParameterSettings settings,
        PsiElement expression
    ) {
        ...
        final GrExpression anchor =
            getAnchorForArgument(oldArgs, toReplaceIn.isVarArgs(), toReplaceIn.getParameterList());
        ...
        final GrClosureSignatureUtil.ArgInfo<PsiElement>[] actualArgs = GrClosureSignatureUtil.mapParametersToArguments(
            signature,
        ...
        if (PsiTreeUtil.isAncestor(toReplaceIn, callExpression, false)) {
            ...
            PsiElement initializer =
                ExpressionConverter.getExpression(expression, GroovyLanguage.INSTANCE, settings.getProject());
    ...
    private static void removeParametersFromCall(
        ...
        final GrClosureSignature signature = GrClosureSignatureUtil.createSignature(
            (PsiMethod)resolved,
    ...
    protected String getCommandName() {
        return RefactoringBundle.message(
            ...
            DescriptiveNameUtil.getDescriptiveName(mySettings.getToReplaceIn())
        );
    }
}
