...
public abstract class GroovyStringLexerBase extends LexerBase {
  ...
  public void start(@Nonnull CharSequence buffer, int startOffset, int endOffset, int initialState) {
    if (buffer.length() < endOffset) {
      LOG.error("buffer Length: " + buffer.length() + ", endOffset: " + endOffset, AttachmentFactory.get().create("",buffer.toString()));
    }
  ...
  private IElementType locateToken() {
    ...
    while (myEnd < myBufferEnd && !checkForSimpleValidEscape(myEnd) && !checkForHexCodeStart(myEnd)) {
      myEnd++;
    }
