...
public class EachWithIndexClosureCompleter extends ClosureCompleter {
  ...
  protected List<ClosureParameterInfo> getParameterInfos(InsertionContext context,
    ...
    PsiType collection = substitutor.substitute(type);

    PsiType iterable = getIteratedType(place, collection);
    if (iterable != null) {
    ...
    if (InheritanceUtil.isInheritor(collection, CommonClassNames.JAVA_UTIL_MAP)) {
      ...
      PsiClass entry = JavaPsiFacade.getInstance(project).findClass(CommonClassNames.JAVA_UTIL_MAP_ENTRY, place.getResolveScope());
      if (entry == null) return null;

      PsiClassType entryType = JavaPsiFacade.getElementFactory(project).createType(entry, typeParams);
  ...
  private static PsiType getIteratedType(PsiElement parent, PsiType collection) {
    if (parent instanceof GrReferenceExpression) {
      GrExpression qualifier = ((GrReferenceExpression)parent).getQualifier();
      if (qualifier != null) {
        return ClosureParameterEnhancer.findTypeForIteration(qualifier, parent);
      }
    }

    PsiType iterable = PsiUtil.extractIterableTypeParameter(collection, true);
