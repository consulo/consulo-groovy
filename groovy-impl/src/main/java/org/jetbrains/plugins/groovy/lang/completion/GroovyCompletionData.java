...
public class GroovyCompletionData
{
	...
	public static final ElementPattern<PsiElement> IN_CAST_TYPE_ELEMENT = StandardPatterns.or(PsiJavaPatterns.psiElement().afterLeaf(PsiJavaPatterns.psiElement().withText("(").withParent
			(PsiJavaPatterns.psiElement(GrParenthesizedExpression.class, GrTypeCastExpression.class))), PsiJavaPatterns.psiElement().afterLeaf(PsiJavaPatterns.psiElement().withElementType
			(GroovyTokenTypes.kAS).withParent(GrSafeCastExpression.class)));
	...
	public static void addGroovyKeywords(CompletionParameters parameters, CompletionResultSet result)
	{
		...
		if(!PlatformPatterns.psiElement().afterLeaf(".", ".&", "@", "*.", "?.").accepts(position))
		{
			...
			if(PsiJavaPatterns.psiElement(GrReferenceExpression.class).inside(StandardPatterns.or(PsiJavaPatterns.psiElement(GrWhileStatement.class),
					PsiJavaPatterns.psiElement(GrForStatement.class))).accepts(parent))
			{
	...
	{
		ElementPattern<PsiElement> forParameter = PsiJavaPatterns.psiElement().withParents(GrParameter.class, GrTraditionalForClause.class, GrForStatement.class);
		return PsiJavaPatterns.psiElement().withParent(GrReferenceExpression.class).afterLeaf(forParameter).accepts(position) || forParameter.accepts(position) && PsiJavaPatterns.psiElement()
				.afterLeaf(PsiJavaPatterns.psiElement(GroovyTokenTypes.mIDENT)).accepts(position);
	}
	...
	private static boolean suggestPackage(PsiElement context)
	{
				...
				((GroovyFile) context.getParent().getParent()).getPackageDefinition() == null)
		{
			...
			if(context.getParent() instanceof GrApplicationStatement && ((GrApplicationStatement) context.getParent()).getExpressionArguments()[0] instanceof GrReferenceExpression)
			{
				return true;
			}
			return false;
	...
	private static boolean afterAtInType(PsiElement context)
	{
		...
		if(previous != null &&
				...
				(context.getParent() != null && context.getParent().getParent() instanceof GroovyFile || context.getParent() instanceof GrCodeReferenceElement && context.getParent().getParent()
	...
	private static boolean suggestPrimitiveTypes(PsiElement context)
	{
		...
		if((parent instanceof GrParameter && ((GrParameter) parent).getTypeElementGroovy() == null) || parent instanceof GrReferenceElement &&
	...
	private static boolean isInfixOperatorPosition(PsiElement context)
	{
		...
		if(context.getParent() instanceof PsiErrorElement && GroovyCompletionUtil.endsWithExpression(GroovyCompletionUtil.nearestLeftSibling(context.getParent())))
		{
			return true;
		}
	...
	private static boolean suggestModifiers(PsiElement context)
	{
		...
		if(GroovyCompletionUtil.isFirstElementAfterPossibleModifiersInVariableDeclaration(context, false) && !PsiJavaPatterns.psiElement().afterLeaf("def").accepts(context))
		{
			return true;
		}
