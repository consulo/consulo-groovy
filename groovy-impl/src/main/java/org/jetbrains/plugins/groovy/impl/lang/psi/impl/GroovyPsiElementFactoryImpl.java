...
public class GroovyPsiElementFactoryImpl extends GroovyPsiElementFactory {
  ...
  @Override
  public GrCodeReferenceElement createReferenceElementFromText(String refName, PsiElement context) {
  ...
  @Override
  @Nonnull
  public GrExpression createExpressionFromText(@Nonnull String text, PsiElement context) {
  ...
  public PsiTypeParameter createTypeParameter(String name, PsiClassType[] superTypes) {
    ...
    if (superTypes.length > 1 || superTypes.length == 1 && !superTypes[0].equalsToText(CommonClassNames
                                                                                         .JAVA_LANG_OBJECT)) {
  ...
  public GrVariableDeclaration createVariableDeclaration(@Nullable String[] modifiers,
    ...
    if (initializer != null) {
      if (initializer instanceof GrApplicationStatement && !GroovyConfigUtils.getInstance().isVersionAtLeast
        (initializer, GroovyConfigUtils.GROOVY1_8, false)) {
  ...
  public GrVariableDeclaration createVariableDeclaration(@Nullable String[] modifiers,
    ...
    if (type != null && type != PsiType.NULL) {
      ...
      String typeText = getTypeText(unboxed);
  ...
  public GrVariableDeclaration createFieldDeclaration(String[] modifiers,
                                                      ...
                                                      @Nullable GrExpression initializer,
                                                      @Nullable PsiType type) {
    String varDeclaration = createVariableDeclaration(modifiers, initializer, type, identifier).getText();

    GroovyFileBase file = createGroovyFileChecked("class A { " + varDeclaration + "}");
    GrTypeDefinitionBody body = file.getTypeDefinitions()[0].getBody();
    LOG.assertTrue(body.getMemberDeclarations().length == 1 && body.getMemberDeclarations()[0] instanceof
      GrVariableDeclaration, "ident = <" + identifier + "> initializer = " + (initializer == null ? "_null_"
      : ("<" + initializer.getText()) + ">"));
  ...
  public GrVariableDeclaration createFieldDeclarationFromText(String text) {
    GroovyFile file = createGroovyFileChecked("class X{\n" + text + "\n}");
  ...
  private static String getTypeText(PsiType type) {
    if (!(type instanceof PsiArrayType)) {
      ...
      String text = canonical != null ? canonical : type.getPresentableText();
      if ("null".equals(text)) {
        return "";
      }
      else {
  ...
  @Override
  public GrClosableBlock createClosureFromText(String closureText,
                                               PsiElement context) throws IncorrectOperationException {
    GroovyFile psiFile = createGroovyFileChecked("def __hdsjfghk_sdhjfshglk_foo  = " + closureText, false,
                                                 context);
  ...
  private GroovyFileImpl createDummyFile(CharSequence text, boolean physical) {
    String fileName = DUMMY_FILE_NAME + '.' + GroovyFileType.GROOVY_FILE_TYPE.getDefaultExtension();
    long stamp = System.currentTimeMillis();
    PsiFileFactory factory = PsiFileFactory.getInstance(myProject);
    return (GroovyFileImpl)factory.createFileFromText(fileName, GroovyFileType.GROOVY_FILE_TYPE, text, stamp,
                                                      physical);
  }
  ...
  public GrParameter createParameter(String name,
    ...
    try {
      ...
      throw new IncorrectOperationException("name = " + name + ", type = " + typeText + ", " +
  ...
  public GrCodeReferenceElement createTypeOrPackageReference(String qName) {
    try {
      GroovyFileBase file = createGroovyFileChecked("def i = new " + qName + "()");
  ...
  public GrTypeDefinition createTypeDefinition(String text) throws IncorrectOperationException {
    GroovyFileBase file = createGroovyFileChecked(text);
  ...
  @Override
  @Nonnull
  public GrTypeElement createTypeElement(String typeText, PsiElement context) throws
    IncorrectOperationException {
    GroovyFile file = createGroovyFileChecked("def " + typeText + " someVar", false, context);
  ...
  @Nonnull
  @Override
  public PsiClassType createType(@Nonnull PsiClass aClass) {
  ...
  public PsiElement createModifierFromText(String name) {
    GroovyFileBase file = createGroovyFileChecked(name + " foo() {}");
  ...
  public GrCodeBlock createMethodBodyFromText(String text) {
    GroovyFileBase file = createGroovyFileChecked("def foo () {" + text + "}");
  ...
  public GrReferenceElement createPackageReferenceElementFromText(String newPackageName) {
    return ((GrPackageDefinition)createGroovyFileChecked("package " + newPackageName).getTopStatements()[0])
      .getPackageReference();
  }
  ...
  public GrMethod createConstructorFromText(@Nonnull String constructorName,
                                            ...
                                            String body,
                                            PsiElement context) {
    CharSequence text = generateMethodText(null, constructorName, null, paramTypes, paramNames, body, true);
    return createConstructorFromText(constructorName, text, context);
  }

  @Override
  public GrMethod createConstructorFromText(String constructorName,
                                            CharSequence constructorText,
                                            @Nullable PsiElement context) {
    GroovyFile file = createGroovyFileChecked("class " + constructorName + "{" + constructorText + "}", false,
                                              context);
    GrTopLevelDefinition definition = file.getTopLevelDefinitions()[0];

    if (!(definition != null && definition instanceof GrClassDefinition)) {
      throw new IncorrectOperationException("constructorName: " + constructorName + ", " +
  ...
  @Override
  @Nonnull
  public GrMethod createMethodFromText(@Nonnull String methodText, @Nullable PsiElement context) {
  ...
  @Nonnull
  @Override
  public GrAnnotation createAnnotationFromText(@Nonnull @NonNls String annotationText,
                                               @Nullable PsiElement context) throws IncorrectOperationException {
    return createMethodFromText(annotationText + " void ___shdjklf_pqweirupncp_foo() {}",
                                context).getModifierList().getRawAnnotations()[0];
  }
  ...
  private GroovyFile createGroovyFileChecked(@Nonnull CharSequence idText,
                                             boolean isPhysical,
                                             @Nullable PsiElement context) {
  ...
  @Override
  public GrNamedArgument createNamedArgument(@Nonnull String name, GrExpression expression) {
  ...
  public GrCodeReferenceElement createCodeReferenceElementFromClass(PsiClass aClass) {
    if (aClass instanceof PsiAnonymousClass) {
      throw new IncorrectOperationException("cannot create code reference element for anonymous class " + aClass
        .getText());
    }
  ...
  public GrModifierList createModifierList(CharSequence text) {
    GrMethod method = createMethodFromText(text + " void foo()");
    return method.getModifierList();
  }

  @Override
  public GrCaseSection createSwitchSection(String text) {
    GrStatement statement = createStatementFromText("switch (a) {\n" + text + "\n}");
  ...
  @Override
  public GrImportStatement createImportStatementFromText(@Nonnull String qName,
                                                         boolean isStatic,
                                                         boolean isOnDemand,
                                                         String alias) {
  ...
  @Override
  public GrImportStatement createImportStatementFromText(@Nonnull String text) {
    PsiFile dummyFile = createGroovyFileChecked(text);
    return ((GrImportStatement)dummyFile.getFirstChild());
  }

  @Override
  public GrImportStatement createImportStatement(@Nonnull String qname,
                                                 boolean isStatic,
                                                 boolean isOnDemand,
                                                 String alias,
                                                 PsiElement context) {
  ...
  @Override
  public GrMethod createMethodFromText(String modifier,
                                       String name,
                                       @Nullable String type,
                                       String[] paramTypes,
                                       PsiElement context) {
    ...
    String[] paramNames = GroovyNamesUtil.getMethodArgumentsNames(myProject, res.toArray(PsiType.createArray(res
                                                                                                               .size())));
    CharSequence text = generateMethodText(modifier, name, type, paramTypes, paramNames, null, false);
    return createMethodFromText(text.toString(), context);
  }

  @Override
  @Nonnull
  public GrDocComment createDocCommentFromText(@Nonnull String text) {
    return (GrDocComment)createGroovyFileChecked(text).getFirstChild();
  }

  @Override
  public GrDocTag createDocTagFromText(String text) {
    GrDocComment docComment = createDocCommentFromText("/**" + text + "*/");
  ...
  public PsiReferenceList createThrownList(PsiClassType[] exceptionTypes) {
    ...
    String end = StringUtil.join(types, ",");
  ...
  public GrCatchClause createCatchClause(PsiClassType type, String parameterName) {
    ...
    GrTryCatchStatement statement = (GrTryCatchStatement)createStatementFromText(buffer);
  ...
  public GrExtendsClause createExtendsClause() {
    GrTypeDefinition typeDefinition = createTypeDefinition("class A extends B {}");
  ...
  public GrImplementsClause createImplementsClause() {
    GrTypeDefinition typeDefinition = createTypeDefinition("class A implements B {}");
  ...
  public GrLiteral createLiteralFromValue(@Nullable Object value) {
    if (value instanceof String) {
      ...
      GrExpression expr = createExpressionFromText(buffer);
  ...
  @Nonnull
  @Override
  public PsiClass createClass(@NonNls @Nonnull String name) throws IncorrectOperationException {
    return createTypeDefinition("class " + name + "{}");
  }

  @Nonnull
  @Override
  public PsiClass createInterface(@NonNls @Nonnull String name) throws IncorrectOperationException {
    return createTypeDefinition("interface " + name + "{}");
  }

  @Nonnull
  @Override
  public PsiClass createEnum(@Nonnull @NonNls String name) throws IncorrectOperationException {
    return createTypeDefinition("enum " + name + "{}");
  }

  @Nonnull
  @Override
  public GrField createField(@Nonnull @NonNls String name, @Nonnull PsiType type) throws IncorrectOperationException {
    GrVariableDeclaration fieldDeclaration = createFieldDeclaration(ArrayUtil.EMPTY_STRING_ARRAY, name,
                                                                          null, type);
  ...
  @Nonnull
  @Override
  public GrMethod createMethod(@Nonnull @NonNls String name,
  ...
  public GrClassInitializer createClassInitializer() throws IncorrectOperationException {
    GrTypeDefinition typeDefinition = createTypeDefinition("class X {{}}");
    return typeDefinition.getInitializers()[0];
  }

  @Nonnull
  @Override
  public GrParameter createParameter(@Nonnull @NonNls String name,
  ...
  public GrParameter createParameter(@Nonnull @NonNls String name,
                                     @Nullable PsiType type,
                                     PsiElement context) throws IncorrectOperationException {
    return createParameter(name, type == null ? null : type.getCanonicalText(), null,
                           context instanceof GroovyPsiElement ? (GroovyPsiElement)context : null);
  }

  @Nonnull
  @Override
  public PsiParameterList createParameterList(@Nonnull @NonNls String[] names,
  ...
  @Nonnull
  @Override
  public PsiMethod createConstructor(@Nonnull @NonNls String name) {
  ...
  @Nonnull
  @Override
  public PsiClassType createType(@Nonnull PsiClass resolve,
  ...
  @Nonnull
  @Override
  public PsiClassType createType(@Nonnull PsiClass aClass, PsiType parameters) {
  ...
  @Nonnull
  @Override
  public PsiSubstitutor createRawSubstitutor(@Nonnull PsiTypeParameterListOwner owner) {
    return JavaPsiFacade.getElementFactory(myProject).createRawSubstitutor(owner);
  }

  @Nonnull
  @Override
  public PsiSubstitutor createSubstitutor(@Nonnull Map<PsiTypeParameter, PsiType> map) {
    return JavaPsiFacade.getElementFactory(myProject).createSubstitutor(map);
  }

  @Override
  public PsiPrimitiveType createPrimitiveType(@Nonnull String text) {
    return JavaPsiFacade.getElementFactory(myProject).createPrimitiveType(text);
  }

  @Nonnull
  @Override
  public PsiClassType createTypeByFQClassName(@Nonnull @NonNls String qName) {
    return JavaPsiFacade.getElementFactory(myProject).createTypeByFQClassName(qName);
  }

  @Nonnull
  @Override
  public PsiClassType createTypeByFQClassName(@Nonnull @NonNls String qName, @Nonnull GlobalSearchScope resolveScope) {
