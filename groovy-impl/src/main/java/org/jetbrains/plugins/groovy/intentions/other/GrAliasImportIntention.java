		...
		manager.startTemplate(newEditor, built, new TemplateEditingAdapter()
		{
			...
			public void templateFinished(Template template, boolean brokenOff)
			{
																													...
																													Computable<GrImportStatement>()
				{
					...
					{
						return PsiTreeUtil.findElementOfClassAtOffset(file, range.getStartOffset(),
								GrImportStatement.class, true);
					}
				});
				...
				ApplicationManager.getApplication().runWriteAction(new Runnable()
				{
					...
					public void run()
					{
						final GrImportStatement context = PsiTreeUtil.findElementOfClassAtRange(file,
								contextImportPointer.getStartOffset(), contextImportPointer.getEndOffset(),
