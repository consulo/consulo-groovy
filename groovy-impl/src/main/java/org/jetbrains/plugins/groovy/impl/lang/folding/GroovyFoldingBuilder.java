...
package org.jetbrains.plugins.groovy.impl.lang.folding;
...
public class GroovyFoldingBuilder extends CustomFoldingBuilder implements DumbAware {
  ...
  private void appendDescriptors(PsiElement element,
                                 List<FoldingDescriptor> descriptors,
                                 Set<PsiElement> usedComments) {
    ...
    if (TokenSets.BLOCK_SET.contains(type) && !isSingleHighLevelClassBody(element) || type == GroovyElementTypes
      .CLOSABLE_BLOCK) {
        ...
        if (lbrace != null && rbrace != null) {
          ...
          if (next != null && PsiImplUtil.isWhiteSpaceOrNls(next) &&
            ...
            descriptors.add(new NamedFoldingDescriptor(psi, prev.getTextRange().getStartOffset(),
                                                       rbrace.getTextRange().getEndOffset(), group, "}"));
            return;
          }
        }
      }
  ...
  private static void addFoldingForStrings(List<FoldingDescriptor> descriptors, ASTNode node) {
    ...
    if (!node.getElementType().equals(GroovyElementTypes.GSTRING) && !node.getElementType().equals
    ...
    if (startOffset + 1 < injectionRange.getStartOffset()) {
      descriptors.add(new NamedFoldingDescriptor(node, startOffset, injectionRange.getStartOffset(), group,
                                                 start_quote));
    }
    ...
    if (startOffset + 1 < nodeRange.getEndOffset()) {
      descriptors.add(new NamedFoldingDescriptor(node.getLastChildNode(), startOffset, nodeRange.getEndOffset(),
                                                 group, end_quote));
    }
  }
  ...
  private static void processImports(final List<FoldingDescriptor> descriptors, GrImportStatement[] imports) {
    ...
    while (first != null) {
      ...
      if (marker != first) {
        ...
        if (start + tail < end && !JavaFoldingBuilderBase.hasErrorElementsNearby(first.getContainingFile(),
                                                                                 start, end)) {
  ...
  protected boolean isRegionCollapsedByDefault(@Nonnull ASTNode node) {
    ...
    if (node.getElementType() == GroovyDocElementTypes.GROOVY_DOC_COMMENT || node.getElementType() ==
    ...
    if ((node.getElementType() == GroovyElementTypes.OPEN_BLOCK || node.getElementType() == GroovyElementTypes
      .CONSTRUCTOR_BODY) && node.getTreeParent().getElementType() == GroovyElementTypes.METHOD_DEFINITION) {
