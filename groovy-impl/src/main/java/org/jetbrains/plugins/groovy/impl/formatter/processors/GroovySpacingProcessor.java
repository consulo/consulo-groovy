...
import org.jetbrains.plugins.groovy.impl.formatter.blocks.ParameterListBlock;
...
public class GroovySpacingProcessor extends GroovyElementVisitor
{
	...
	public GroovySpacingProcessor(GroovyBlock block1, GroovyBlock block2, FormattingContext context)
	{
		...
		if(manageMethodParameterList(block2))
		{
			return;
		}
	...
	public void visitArgumentList(GrArgumentList list)
	{
		...
		{
			processParentheses(GroovyTokenTypes.mLPAREN, GroovyTokenTypes.mRPAREN,
					mySettings.SPACE_WITHIN_METHOD_CALL_PARENTHESES,
					mySettings.SPACE_WITHIN_EMPTY_METHOD_CALL_PARENTHESES,
					mySettings.CALL_PARAMETERS_LPAREN_ON_NEXT_LINE, mySettings.CALL_PARAMETERS_RPAREN_ON_NEXT_LINE);
		}
	}
			...
			@Nonnull final TextRange range)
	{
		if(isLineFeed)
		{
			myResult = Spacing.createDependentLFSpacing(isSpace ? 1 : 0, isSpace ? 1 : 0, range,
					mySettings.KEEP_LINE_BREAKS, keepBlankLines());
		}
		else
		{
	...
	public void visitFile(GroovyFileBase file)
	{
		...
		{
			myResult = Spacing.createSpacing(0, 0, mySettings.BLANK_LINES_AFTER_PACKAGE + 1,
					mySettings.KEEP_LINE_BREAKS, keepBlankLines());
		}
		else if(myType2 == GroovyElementTypes.PACKAGE_DEFINITION)
		{
			myResult = Spacing.createSpacing(0, 0, mySettings.BLANK_LINES_BEFORE_PACKAGE + 1,
					mySettings.KEEP_LINE_BREAKS, keepBlankLines());
		}
		else if(isLeftOrRight(TokenSets.TYPE_DEFINITIONS))
		{
			...
			{
				myResult = Spacing.createSpacing(0, 0, mySettings.BLANK_LINES_AROUND_CLASS + 1,
						mySettings.KEEP_LINE_BREAKS, keepBlankLines());
			}
		}
		else if(isAfterElementOrSemi(GroovyElementTypes.IMPORT_STATEMENT) && myType2 != GroovyElementTypes
				.IMPORT_STATEMENT)
		{ //after imports
			myResult = Spacing.createSpacing(0, 0, mySettings.BLANK_LINES_AFTER_IMPORTS + 1,
					mySettings.KEEP_LINE_BREAKS, keepBlankLines());
		}
		else if(myType1 != GroovyElementTypes.IMPORT_STATEMENT && !isSemiAfter(GroovyElementTypes.IMPORT_STATEMENT) &&
				myType2 == GroovyElementTypes.IMPORT_STATEMENT)
		{ //before imports
			myResult = Spacing.createSpacing(0, 0, mySettings.BLANK_LINES_BEFORE_IMPORTS, mySettings.KEEP_LINE_BREAKS,
					keepBlankLines());
		}
		else if(isAfterElementOrSemi(GroovyElementTypes.IMPORT_STATEMENT) && myType2 == GroovyElementTypes
				.IMPORT_STATEMENT)
	...
	private boolean isSemiAfter(@Nonnull IElementType statement)
	{
		return myType1 == GroovyTokenTypes.mSEMI && getStatementTypeBySemi(myChild1) == statement;
	}

	private boolean isSemiAfter(@Nonnull TokenSet set)
	{
        ...
        */
		...
		{  //empty closure
			createSpaceInCode(false);
				...
				GroovyTokenTypes.mCLOSABLE_BLOCK_OP || myType2 == GroovyTokenTypes.mRCURLY)
		{ //spaces between statements
			boolean spacesWithinBraces = closure.getParent() instanceof GrStringInjection ? myGroovySettings
					.SPACE_WITHIN_GSTRING_INJECTION_BRACES : mySettings.SPACE_WITHIN_BRACES;
			createDependentLFSpacing(true, spacesWithinBraces, closure.getTextRange());
		}
		else if(myType1 == GroovyTokenTypes.mCLOSABLE_BLOCK_OP)
		{
			myResult = GroovySpacingProcessorBasic.createDependentSpacingForClosure(mySettings, myGroovySettings,
					closure, true);
		}
		else if(myType1 == GroovyTokenTypes.mLCURLY && (myType2 == GroovyElementTypes.PARAMETERS_LIST || myType2 ==
	...
	public void visitOpenBlock(GrOpenBlock block)
	{
		...
		boolean keepInOneLine = isMethod ? mySettings.KEEP_SIMPLE_METHODS_IN_ONE_LINE : mySettings
	...
	public void visitTypeDefinition(GrTypeDefinition typeDefinition)
	{
		if(myType2 == GroovyElementTypes.CLASS_BODY)
		{
			...
			{
				createSpaceProperty(mySettings.SPACE_BEFORE_CLASS_LBRACE, true, 100); //don't manually remove line
				// feeds because this line is ambiguous
			}
			else
			{
	...
	private void processClassMembers(@Nullable GrTypeDefinitionBody typeDefinitionBody)
	{
		...
		{ //check variable definitions only inside class body
		...
		if(TokenSets.METHOD_DEFS.contains(myType1) || isSemiAfter(TokenSets.METHOD_DEFS) || TokenSets.METHOD_DEFS
				.contains((myType2)))
		{
			if(myType1 == GroovyDocElementTypes.GROOVY_DOC_COMMENT)
			{
		...
		if(TokenSets.TYPE_DEFINITIONS.contains(myType1) || isSemiAfter(TokenSets.TYPE_DEFINITIONS) || TokenSets
				.TYPE_DEFINITIONS.contains((myType2)))
		{
			if(myType1 == GroovyDocElementTypes.GROOVY_DOC_COMMENT)
			{
				createLF(true);
	...
	public void visitTypeParameterList(GrTypeParameterList list)
	{
		...
		else if(myType1 == GroovyTokenTypes.mLT && myType2 == GroovyElementTypes.TYPE_PARAMETER || myType1 ==
				GroovyElementTypes.TYPE_PARAMETER && myType2 == GroovyTokenTypes.mGT)
		{
			createSpaceInCode(false);
		}
	}
	...
	public void visitMethod(GrMethod method)
	{
		...
		else if(isOpenBlock(myType2))
		{
			...
			int dependencyStart = methodName == null ? myParent.getTextRange().getStartOffset() : methodName
		...
		{
			processParentheses(GroovyTokenTypes.mLPAREN, GroovyTokenTypes.mRPAREN,
					mySettings.SPACE_WITHIN_METHOD_PARENTHESES, mySettings.SPACE_WITHIN_EMPTY_METHOD_PARENTHESES,
	...
	{
		if(myType1 == GroovyDocTokenTypes.mGDOC_TAG_VALUE_LPAREN || myType2 == GroovyDocTokenTypes
				.mGDOC_TAG_VALUE_RPAREN || myType2 == GroovyDocTokenTypes.mGDOC_TAG_VALUE_COMMA)
		{
			createSpaceProperty(false, false, 0);
		}
		else
	...
	public void visitTryStatement(GrTryCatchStatement tryCatchStatement)
	{
		...
		{
			createSpaceBeforeLBrace(mySettings.SPACE_BEFORE_TRY_LBRACE, mySettings.BRACE_STYLE, null,
					mySettings.KEEP_SIMPLE_BLOCKS_IN_ONE_LINE);
		}
	...
	public void visitBinaryExpression(GrBinaryExpression expression)
	{
		...
		@SuppressWarnings("SimplifiableConditionalExpression") boolean spaceAround = isLeftOrRight(SpacingTokens
				.LOGICAL_OPERATORS) ? mySettings.SPACE_AROUND_LOGICAL_OPERATORS : isLeftOrRight(SpacingTokens
				.EQUALITY_OPERATORS) ? mySettings.SPACE_AROUND_EQUALITY_OPERATORS : isLeftOrRight(SpacingTokens
				.RELATIONAL_OPERATORS) ? mySettings.SPACE_AROUND_RELATIONAL_OPERATORS : isLeftOrRight(SpacingTokens
				.BITWISE_OPERATORS) ? mySettings.SPACE_AROUND_BITWISE_OPERATORS : isLeftOrRight(SpacingTokens
				.ADDITIVE_OPERATORS) ? mySettings.SPACE_AROUND_ADDITIVE_OPERATORS : isLeftOrRight(SpacingTokens
				.MULTIPLICATIVE_OPERATORS) ? mySettings.SPACE_AROUND_MULTIPLICATIVE_OPERATORS : isLeftOrRight
				(SpacingTokens.SHIFT_OPERATORS) ? mySettings.SPACE_AROUND_SHIFT_OPERATORS : isLeftOrRight
				(SpacingTokens.REGEX_OPERATORS) ? myGroovySettings.SPACE_AROUND_REGEX_OPERATORS : isLeftOrRight
				(GroovyTokenTypes.kIN);
		if(TokenSets.BINARY_OP_SET.contains(myType2))
		{
			createDependentLFSpacing(mySettings.BINARY_OPERATION_SIGN_ON_NEXT_LINE, spaceAround,
	...
	{
		if(!expression.isPostfix() && expression.getOperationToken() == myChild1 || expression.isPostfix() &&
				expression.getOperationToken() == myChild2)
		{
			createSpaceInCode(mySettings.SPACE_AROUND_UNARY_OPERATOR);
		}
	}
	...
	public void visitDocComment(GrDocComment comment)
	{
		...
		if(myType1 == GroovyDocTokenTypes.mGDOC_COMMENT_START && myType2 == GroovyDocTokenTypes.mGDOC_COMMENT_DATA ||
				myType1 == GroovyDocTokenTypes.mGDOC_COMMENT_DATA && myType2 == GroovyDocTokenTypes
						.mGDOC_COMMENT_END ||
	...
	{
		if(myType1 == GroovyDocTokenTypes.mGDOC_INLINE_TAG_START || myType2 == GroovyDocTokenTypes
	...
	{
		processParentheses(GroovyTokenTypes.mLPAREN, GroovyTokenTypes.mRPAREN, mySettings.SPACE_WITHIN_PARENTHESES,
				null, mySettings.PARENTHESES_EXPRESSION_LPAREN_WRAP, mySettings.PARENTHESES_EXPRESSION_RPAREN_WRAP);
	}
	...
	public void visitIfStatement(GrIfStatement ifStatement)
	{
		...
		{
			boolean space = myChild2.getPsi() == ((GrIfStatement) myParent).getElseBranch() ? mySettings
					.SPACE_BEFORE_ELSE_LBRACE : mySettings.SPACE_BEFORE_IF_LBRACE;
			createSpaceBeforeLBrace(space, mySettings.BRACE_STYLE, new TextRange(myParent.getTextRange()
					.getStartOffset(), myChild1.getTextRange().getEndOffset()),
					mySettings.KEEP_SIMPLE_BLOCKS_IN_ONE_LINE);
		}
		else if(myType2 == GroovyTokenTypes.mLPAREN)
		{
	...
	public void visitForStatement(GrForStatement forStatement)
	{
		...
		{
			processParentheses(GroovyTokenTypes.mLPAREN, GroovyTokenTypes.mRPAREN,
					mySettings.SPACE_WITHIN_FOR_PARENTHESES, null, mySettings.FOR_STATEMENT_LPAREN_ON_NEXT_LINE,
			...
			boolean keepOneLine)
	{
		...
		else if(braceStyle == CommonCodeStyleSettings.END_OF_LINE || braceStyle == CommonCodeStyleSettings
		...
		{
			int space = spaceBeforeLbrace ? 1 : 0;
			myResult = Spacing.createDependentLFSpacing(space, space, myParent.getTextRange(),
					mySettings.KEEP_LINE_BREAKS, keepBlankLines());
		}
		else
		{
