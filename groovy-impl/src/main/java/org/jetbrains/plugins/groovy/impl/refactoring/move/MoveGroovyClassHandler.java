...
import com.intellij.java.impl.refactoring.move.moveClassesOrPackages.MoveClassHandler;
...
public class MoveGroovyClassHandler implements MoveClassHandler {
  ...
  public PsiClass doMoveClass(@Nonnull PsiClass aClass,
    ...
    if (aClass instanceof GroovyScriptClass) {
      ...
      if (!moveDestination.equals(file.getContainingDirectory()) && moveDestination.findFile(file.getName()) !=
        null) {
  ...
  public String getName(PsiClass clazz) {
    ...
    return ((GroovyFile)file).getClasses().length > 1 ? clazz.getName() + "." + GroovyFileType.DEFAULT_EXTENSION
      : file.getName();
  }
