...
public abstract class GrIntroduceHandlerBase<Settings extends GrIntroduceSettings, Scope extends PsiElement> implements RefactoringActionHandler
{
	...
	{
		@Override
		public String fun(@Nonnull GrExpression expr)
		{
			return expr.getText();
		}
	};
	...
	protected abstract GrAbstractInplaceIntroducer<Settings> getIntroducer(@Nonnull GrIntroduceContext context, OccurrencesChooser.ReplaceChoice choice);
			...
			@Nullable final StringPartInfo stringPart)
	{
		...
		if(scopes.length == 0)
		{
			CommonRefactoringUtil.showErrorHint(project, editor, RefactoringBundle.getCannotRefactorMessage(getRefactoringName() + "is not available in current scope"), getRefactoringName(),
					getHelpID());
		}
			...
			public void run()
			{
				ApplicationManager.getApplication().runWriteAction(new Runnable()
				{
					...
					public void run()
					{
						...
						contextRef.set(new GrIntroduceContextImpl(context.getProject(), context.getEditor(), expressionRef != null ? expressionRef.getElement() : null,
	...
	private void invoke(@Nonnull final Project project, @Nonnull final Editor editor, @Nonnull PsiFile file, int startOffset, int endOffset) throws GrRefactoringError
	{
		try
		{
	...
	public static boolean hasLhs(@Nonnull final PsiElement[] occurrences)
	{
