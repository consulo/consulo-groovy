...
public class GroovyPositionManager implements PositionManager
{
	...
	public List<Location> locationsOfLine(@Nonnull ReferenceType type, @Nonnull SourcePosition position) throws NoDataException
	{
		try
		{
			...
			List<Location> locations = getDebugProcess().getVirtualMachineProxy().versionHigher("1.4") ? type.locationsOfLine(DebugProcess.JAVA_STRATUM, null, line) : type.locationsOfLine(line);
	...
	@Nullable
	private static PsiFile getExtraScriptIfNotFound(Project project, ReferenceType refType, String runtimeName, GlobalSearchScope searchScope)
	{
		for(ScriptPositionManagerHelper helper : ScriptPositionManagerHelper.EP_NAME.getExtensions())
		{
